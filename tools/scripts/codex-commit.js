...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    if (token === '--human-override') {
      args.humanOverride = String(argv[i + 1] || '').trim().toLowerCase();
      i += 1;
      continue;
    }
    if (token.startsWith('--human-override=')) {
      args.humanOverride = token.slice('--human-override='.length).trim().toLowerCase();
      continue;
    }
...
function usage() {
  console.log('Usage: node tools/scripts/codex-commit.js --message <text> [--no-verify --human-override true --override-note <text>] [--trailer <k=v>]');
}
...
function validateArgs(args) {
  ...
  if (String(process.env.ALLOW_HUMAN_NO_VERIFY || '').trim() !== '1') {
    throw new Error('ALLOW_HUMAN_NO_VERIFY=1 is required for --no-verify override path');
  }
}
