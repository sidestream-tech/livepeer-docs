...
description: >-
  This page provides a comprehensive inventory of all files and folders in the `/snippets` directory. The snippets
  folder is the central location for all custom components, data, scripts, and assets used in the Livepeer
  documentation.
audience: everyone
purpose: concept
keywords:
...
Custom React/JSX components organized by purpose. The active library now uses five production categories plus `_archive/` for preserved legacy modules under review.

#### Primitives (`/snippets/components/primitives/`)
- `buttons.jsx` - Button components (DownloadButton)
...
- `externalContent.jsx` - ExternalContent component for loading external docs
- `math.jsx` - MathInline and MathBlock helpers
- `quote.jsx` - Quote and FrameQuote components
...
- `video.jsx` - Video, embed, and download components
- `zoomableDiagram.jsx` - ScrollableDiagram component

#### Data (`/snippets/components/data/`)
- `coingecko.jsx` - CoinGeckoExchanges component
- `data.jsx` - Blog, forum, events, and social feed layouts
- `embed.jsx` - MarkdownEmbed, TwitterTimeline, and PdfEmbed components
- `showcaseCards.jsx` - ShowcaseCards component
- `videoData.jsx` - YouTubeVideoData helper component

#### Page Structure (`/snippets/components/page-structure/`)
- `frame-mode.jsx` - Frame mode heading and typography components
- `heroGif.jsx` - Starfield animation component
- `portals.jsx` - Portal page layout components

#### Archive (`/snippets/components/_archive/`)
- Preserved legacy source files and retired folders kept for migration review only
...
- Legacy theme styling file - deprecated in favour of CSS Custom Properties in `style.css`
...
- **Components**: ~50+ JSX/TSX files across 5 active categories, plus `_archive/` for migration review
