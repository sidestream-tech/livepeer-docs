...
<Columns cols={3}>
    ...
    <div>
        Related Pages
        - [Run a Gateway](/v2/cn/gateways/quickstart/gateway-setup)
        - [Fund Gateway](./fund-gateway)
        - [Install Gateway](/v2/cn/gateways/run-a-gateway/install/install-overview)
...
<Card
  ...
  href="/v2/cn/gateways/run-a-gateway/install/install-overview"
  icon="tools"
  horizontal
  arrow
>
  ...
  -> then come back to this guide.{' '}
    ...
    Livepeer 将在您首次 [安装您的网关软件](/v2/cn/gateways/run-a-gateway/install/install-overview).
...
<StyledStep title="RPC URL `-ethUrl` flag" titleSize="h2">
    ...
    <Accordion title="See All RPC Options" icon="link">
        <Tabs>
            ...
            - Dynamically Imported: <DoubleIconLink label="Arbitrum RPC Reference" href="/v2/cn/gateways/references/arbitrum-rpc" iconLeft="books" />
...
<StyledStep title="ETH Keystore `-ethKeystorePath`" titleSize="h2">
    ...
    <Tip>
        It's recommended to let Livepeer devtools automatically create the keystore on first run after [installing the Gateway software](/v2/cn/gateways/run-a-gateway/install/install-overview).
    </Tip>
