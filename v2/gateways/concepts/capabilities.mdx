...
BYOC pipelines allow applications to define custom container-based workloads that orchestrators execute. The gateway routes BYOC requests the same way it routes AI inference - via HTTP API - but the orchestrator runs an application-defined container instead of a standard AI model.
