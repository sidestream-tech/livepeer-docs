...
This page covers what a gateway can do. For what a gateway *is*, see [Role](/v2/gateways/concepts/role). For how it works internally, see [Architecture](/v2/gateways/concepts/architecture).
...
4. **Payment handling** - generate probabilistic micropayment tickets per job segment (on-chain mode) or delegate payment to a remote signer (off-chain mode)
...
Gateways can route four categories of work into the Livepeer network. Which workloads you can route depends on your payment mode (on-chain or off-chain).
...
## How gateways choose orchestrators
...
|---|---|
| **Capability** | Does the orchestrator support the requested pipeline, model, or transcoding profile? |
| **Price** | Is the orchestrator's advertised price within the gateway's configured `maxPricePerUnit`? |
| **Latency** | How fast has this orchestrator responded to recent jobs? |
| **Performance history** | What is the orchestrator's success rate and error rate over recent sessions? |
| **Stake weight** | How much LPT is staked to this orchestrator? Higher stake implies greater economic commitment to good behaviour |

### Discovery methods

How a gateway discovers available orchestrators depends on the payment mode:
...
<Tabs>
  ...
  <Tab title="Off-chain gateway">
    ...
    See the <LinkArrow href="/v2/gateways/payments/remote-signers" label="Remote Signers" newline={false} /> page for how discovery works in off-chain mode.
  </Tab>
</Tabs>
...
## The gateway's marketplace role
