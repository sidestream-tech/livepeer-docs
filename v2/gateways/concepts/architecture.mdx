...
### Remote signer (off-chain mode)

In off-chain mode, the gateway delegates all payment operations to a **remote signer** service. The remote signer:
...
### Arbitrum protocol (on-chain mode)

In on-chain mode, the gateway interacts directly with Arbitrum smart contracts:
...
- Payment ticket generation (on-chain mode) or remote signer communication (off-chain mode)
...
### Arbitrum smart contracts (on-chain mode)
