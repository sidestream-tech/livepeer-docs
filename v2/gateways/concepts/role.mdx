...
<Note>
An on-chain gateway can run video, AI, or both workloads - "dual" refers to running both workload types from a single node, not a third operational mode. See <LinkArrow href="/v2/gateways/guides/setup-options/path-requirements#dual-ai-video" label="Dual gateway configuration" newline={false} /> for setup details.
</Note>
