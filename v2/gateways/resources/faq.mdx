...
This page answers "how does X work?" questions. If something is broken, see [Troubleshooting](/v2/gateways/guides/monitoring-troubleshooting/troubleshooting) instead.
...
<AccordionGroup>
  ...
  <Accordion title="I see commands using -broadcaster but my docs say -gateway. Which is correct?">
    ...
    {/* REVIEW: Rick (@rickstaa) — confirm whether -broadcaster is still accepted as an alias in current builds or is fully deprecated */}
  </Accordion>
  ...
  <Accordion title="Do I need ETH or LPT to run a gateway?">
    ...
    If you want to run an AI gateway without managing ETH, see [Payment Paths](/v2/gateways/guides/payments-pricing/payment-paths).
  </Accordion>
...
<AccordionGroup>
  ...
  <Accordion title="What is -maxPricePerUnit and why doesn't it match what I'm paying?">
    ...
    The price you charge *your* customers (if running a commercial gateway service) is entirely separate and managed by your own application layer. See [Pricing Strategy](/v2/gateways/guides/payments-pricing/pricing-strategy).
  </Accordion>
  ...
  <Accordion title="I bridged ETH to Arbitrum but my balance shows zero.">
    ...
    See [Fund Your Gateway](/v2/gateways/guides/payments-pricing/fund-your-gateway) for the full step-by-step.
  </Accordion>
...
<AccordionGroup>
  ...
  <Accordion title="The AI gateway binary doesn't work on Windows. Why?">
    ...
    **Workaround:** Use Docker on Linux (or WSL2 on Windows).
    ...
    {/* REVIEW: Rick — confirm if Windows AI binary is on the roadmap */}
  </Accordion>
  ...
  <Accordion title="What ports does the AI gateway use?">
    | Port | Purpose | Configurable with |
    |------|---------|------------------|
    | `8937` | HTTP AI gateway API | `-httpAddr` |
    | `5935` | CLI management API | `-cliAddr` |

    Note that the AI gateway defaults to `8937`, not `8935` (which is the video gateway default). This is intentional — you can run both on the same machine without port conflicts.
  </Accordion>
  ...
  <Accordion title="How do I find which AI models and pipelines are available?">
    ...
    {/* REVIEW: Rick / j0sh / Peter — confirm /getOrchestratorInfo is the correct and stable endpoint for capability discovery */}
...
|----------|------|----------------|
| Livepeer Docs — Set Pricing | [docs.livepeer.org](https://docs.livepeer.org/orchestrators/orchestrators-portal/guides/set-pricing) | How orchestrators set per-pixel pricing, auto-adjustment |
...
<CardGroup cols={3}>
  ...
  <Card title="Pricing Strategy" icon="circle-dollar-sign" href="/v2/gateways/guides/payments-pricing/pricing-strategy">
...
for port 8935/8937. See v1 troubleshoot for hairpinning details.
...
HTTP: 8937 (different from video 8935). No RTMP. CLI: 5935.
