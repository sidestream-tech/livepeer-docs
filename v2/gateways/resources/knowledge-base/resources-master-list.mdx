...
3. **[Why Run a Gateway](https://docs.livepeer.org/gateways/run-a-gateway/why-run-a-gateway)** — Business case: cost savings at scale, full orchestrator control, lower latency, inference fee capture. Official.
...
12. **[AI Configuration Reference](https://docs.livepeer.org/gateways/run-a-gateway/configure/ai-configuration)** — AI-specific flags: `-aiWorker`, `-orchAddr`, `-remoteSignerAddr`, ports, and startup log verification. Official.
...
23. **[Run a Video Gateway](https://docs.livepeer.org/gateways/run-a-gateway/run-a-gateway)** — Full on-chain video transcoding gateway setup: RTMP input, ETH key, Arbitrum RPC, orchestrator connection. Official.
...
25. **[Video Configuration](https://docs.livepeer.org/gateways/run-a-gateway/configure/video-configuration)** — Video-specific flags: `-broadcaster`, RTMP port, transcode profiles, segment size, and webhook auth config. Official.
26. **[Advanced Transcoding Config](https://docs.livepeer.org/gateways/run-a-gateway/transcoding)** — Output profiles, per-capability pricing, and `setbroadcastconfig` CLI commands for production transcoding. Official.
27. **[Bridge LPT to Arbitrum](https://docs.livepeer.org/gateways/references/bridge-lpt-to-arbitrum)** — How to move LPT from Ethereum mainnet to Arbitrum One for on-chain registration and staking. Official.
...
29. **[Dual Configuration Guide](https://docs.livepeer.org/gateways/run-a-gateway/configure/dual-configuration)** — Run video transcoding and AI inference from the same go-livepeer instance. Port assignments, flag combinations, pitfalls. Official.
...
31. **[Pricing Configuration](https://docs.livepeer.org/gateways/run-a-gateway/configure/pricing-configuration)** — Set `-maxPricePerUnit` and `-maxPricePerCapability`, format the AI pricing JSON, price negotiation between gateway and orchestrators. Official.
...
36. **[Connect with Orchestrator Offerings](https://docs.livepeer.org/gateways/run-a-gateway/connect/connect-with-offerings)** — How an on-chain gateway connects to orchestrators: stake-weighted for video, capability-based for AI. Official.
37. **[Discover Orchestrator Offerings](https://docs.livepeer.org/gateways/run-a-gateway/connect/discover-offerings)** — Browse available orchestrators by capability, model support, and pricing via the Livepeer Marketplace. Official.
38. **[Livepeer Marketplace](https://docs.livepeer.org/gateways/run-a-gateway/connect/lp-marketplace)** — How the on-chain marketplace works: publishing, pricing, and capability advertisement. Official.

### Monitoring and operations

39. **[Monitor and Optimise](https://docs.livepeer.org/gateways/run-a-gateway/monitor/monitor-and-optimise)** — Key metrics, ETH balance monitoring, ticket redemption rates, log patterns for payment and session failures. Official.
...
43. **[Gateway Operator Opportunities](https://docs.livepeer.org/gateways/run-a-gateway/gateway-operator-opportunities)** — Revenue models: inference fees, SPE grants, Foundation programmes, the NaaP approach to building a gateway business. Official.
