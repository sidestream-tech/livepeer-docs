...
<AccordionGroup>
  ...
  <Accordion title="On-chain discovery (-aiServiceRegistry)" icon="link">
    On-chain AI gateways (dual node type) can use the AI service registry for automatic discovery:

    ```bash
    -aiServiceRegistry <REGISTRY_CONTRACT_ADDRESS>
    ```

    {/* REVIEW: confirm -aiServiceRegistry contract address and whether this is active on Arbitrum One with Rick/j0sh. */}

    This queries registered AI-capable orchestrators from the on-chain registry. The gateway's `/getNetworkCapabilities` endpoint exposes the aggregated capability data from discovered orchestrators.
  </Accordion>
</AccordionGroup>
...
<Tabs>
  <Tab title="Off-chain AI (standard)" icon="cloud">
    ...
    {/* REVIEW: confirm remote signer PM path details for off-chain AI with Rick/j0sh - does it use -remoteSigner flag? */}
...
{/* REVIEW: confirm /getNetworkCapabilities endpoint path and response format with Rick/j0sh. */}
