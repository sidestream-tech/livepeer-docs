...
A Job Pipeline is a Gateway **Node Type** or Node Mode and can be configured for Video, AI or Dual purpose workloads (including real-time video AI).
It is the control-plane path between an application request and orchestrator execution.
...
Livepeer supports two main categories of job that a gateway can route. Each has different ingest patterns, payment models, and orchestrator requirements.
The third option - Dual mode support both Video and AI workloads.
