...
<Steps>
  ...
  <Step title="Configure the gateway or SDK">
...
For the NaaP (Network as a Platform) architecture that frames clearinghouse infrastructure, see <LinkArrow href="/v2/gateways/guides/opportunities/naap-multi-tenancy" label="NaaP Multi-Tenancy" newline={false} /> and the NaaP-related proposals on the [Livepeer Forum](https://forum.livepeer.org).
...
{/* REVIEW: Confirm the exact URL is signer.eliteencoder.net. Verify with John at Elite Encoder. */}
...
{/* REVIEW: Confirm tools.livepeer.cloud is the correct active URL and that livepeer.tools is confirmed down (nginx default page as of March 2026). */}
...
<AccordionGroup>
  <Accordion title="What the operator gains" icon="circle-check">
  ...
  <Accordion title="What the operator gives up" icon="circle-xmark">
  ...
  <Accordion title="When to stay self-managed" icon="scale-balanced">
...
<CardGroup cols={2}>
  ...
  <Card title="NaaP Multi-Tenancy" icon="users" href="/v2/gateways/guides/opportunities/naap-multi-tenancy">
    Network as a Platform architecture for multi-tenant gateway operation.
  </Card>
