...
<Tabs>
    <Tab title = "Video Configuration">
        ...
        **Fee calculation:** The effective cap is `maxPricePerUnit` wei per `pixelsPerUnit` pixels. A 1920x1080 frame contains 2,073,600 pixels. With a cap of 1,000 wei per pixel and `pixelsPerUnit` of 1, that frame costs at most 2,073,600 wei (about 0.000002 ETH).
        ...
        To configure caps in USD rather than wei, pass a USD value:
        ...
        {/* REVIEW: Confirm that -priceFeedAddr is required when using USD notation in -maxPricePerUnit. Rick to verify and provide the Chainlink contract address for Arbitrum One. */}
    ...
    <Tab title="AI Configuration">
        ...
        **Pipeline-specific units:**
        ...
        For the `live-video-to-video` pipeline, the gateway sends periodic payments rather than per-frame or per-request. The interval is configured with:
...
{/* REVIEW: Confirm -maxPricePerUnit acts as an AI fallback when -maxPricePerCapability is not set. j0sh to verify. */}
...
<Tabs>
  ...
  <Tab title="Off-chain" icon="cloud">
    **Set the cap based on agreed rates:**
    ...
    3. The cap prevents cost spikes if an Orchestrator raises prices unexpectedly. Jobs fail rather than overpaying.
...
<Tabs>
  ...
  <Tab title="HTTP API" icon="globe">
    ...
    {/* REVIEW: Confirm exact HTTP endpoint paths and request body format for /setBroadcastConfig and /setMaxPriceForCapability. Rick / j0sh to verify current endpoint behaviour. */}
  </Tab>
