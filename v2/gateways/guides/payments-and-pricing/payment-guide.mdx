...
<AccordionGroup>
  <Accordion title="Fastest onboarding, no crypto" icon="rocket">
    Run an AI Gateway in **off-chain mode** with a clearinghouse or the community remote signer for testing. The gateway holds no ETH and no Ethereum key. Once a public clearinghouse reaches GA, this becomes a pure API key sign-up. For now, contact the network community via Discord `#local-gateways` or use the community signer at `signer.eliteencoder.net`.
  ...
  <Accordion title="Live AI with key isolation" icon="shield-halved">
    Use a **remote signer**. A separate service holds the Ethereum key and signs PM tickets. The gateway talks to it over the network and holds no key. Multiple gateway instances can share a single signer.

    Next step: <LinkArrow href="/v2/gateways/guides/payments-and-pricing/remote-signers" label="Remote Signers" newline={false} />
  </Accordion>

  <Accordion title="Gateway-as-a-service for many users" icon="building-columns">
    Build or integrate with a **clearinghouse**. A clearinghouse runs remote signer instances, holds ETH, and adds user management, accounting, and billing. See the Opportunities section for guidance on building Network-as-a-Platform (NaaP) infrastructure.

    Next step: <LinkArrow href="/v2/gateways/guides/payments-and-pricing/clearinghouse-guide" label="Clearinghouses" newline={false} />
  </Accordion>

  <Accordion title="Python, mobile, or browser gateway" icon="code">
    Use the **remote signer path** via the Livepeer SDK. The SDK handles the PM signing interaction with the remote signer. The gateway holds no Ethereum dependency.

    Next step: <LinkArrow href="/v2/gateways/guides/payments-and-pricing/remote-signers" label="Remote Signers" newline={false} />
  </Accordion>

  <Accordion title="Dual gateway (video + AI)" icon="clone">
...
The gateway's operational mode determines whether the gateway process itself handles Ethereum interactions or delegates them.

<StyledTable headers={["", "On-chain", "Off-chain"]}>
  <TableRow>
...
{/* REVIEW: Confirm that off-chain AI gateways truly have zero payment obligation in the gateway process. The design doc implies gateways operating entirely off-chain still use PM tickets via remote signer. Clarify whether "off-chain" means "no Ethereum integration in the gateway process" or "no payments at all". j0sh to verify. */}

<Note>
  **Off-chain does not mean free.** Off-chain gateways still pay Orchestrators via PM tickets - the signing and ETH custody are handled by a remote signer or clearinghouse rather than the gateway process itself.
</Note>
