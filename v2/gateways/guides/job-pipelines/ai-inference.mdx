...
<AccordionGroup>
  <Accordion title="Standard API pipelines" icon="code">
    ...
    | Text to speech | `/text-to-speech` | Audio | {/* REVIEW: confirm pricing unit for text-to-speech with j0sh/Peter */} Audio milliseconds |
    ...
    | LLM | `/llm` | Text | {/* REVIEW: confirm per-token vs per-request pricing for LLM with j0sh */} Per-request or per-token |
...
<CardGroup cols={2}>
  ...
  <Card title="AI API Reference" icon="code" href="/v2/gateways/references/api-reference/AI-API/text-to-image">
