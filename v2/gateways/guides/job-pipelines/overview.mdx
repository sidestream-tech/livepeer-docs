...
A gateway pipeline is the control-plane path between an application request and orchestrator execution. Your gateway does not run compute — it accepts requests, selects compatible orchestrators, enforces routing policy, and returns results. Understanding which pipeline type matches your workload shapes every other configuration decision you make.
...
## What a gateway does in a pipeline
...
| BYOC | No | Yes | Yes | No |
| Live video AI (live-video-to-video) | No | Yes | Yes | {/* REVIEW: on-chain payment applies when run via dual gateway — confirm with Rick/j0sh */} No (off-chain) / Yes (dual on-chain) |
