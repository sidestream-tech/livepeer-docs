...
<AccordionGroup>
  <Accordion title="Funding and deposits" icon="coins">
    **livepeer_cli** Option 1 (Get node status) for the current balance. **Arbiscan** for the full deposit history and on-chain confirmation. See <LinkArrow href="/v2/gateways/guides/monitoring-and-tooling/on-chain-debugging" label="On-Chain Debugging" newline={false} /> for detailed workflows.
  </Accordion>
...
<CustomDivider middleText="TOGGLE VIEWS"/>
<View title="Monitoring Dashboards" icon="gauge-high">
    <BorderedBox variant="accent">
        ...
        ## <Icon icon="computer" size={20} color="var(--accent)" />  Livepeer Tools
...
<View title="On-Chain Monitoring" icon="link">
    <BorderedBox variant="accent">
        ## <Icon icon="ethereum" size={20} color="var(--accent)" /> Arbiscan

        Arbiscan is the block explorer for Arbitrum One - the authoritative source for what has happened on-chain. It covers contract event logs, transaction history, and token transfers.

        <Badge color="blue">Video</Badge> <Badge color="green">Dual</Badge> - applies to on-chain Gateways only. Off-chain AI Gateways have no on-chain activity.

        Key use cases include monitoring deposit and reserve changes, tracking ticket redemptions, detecting unexpected drain, and setting up address watchlist notifications. For detailed on-chain debugging workflows, see <LinkArrow href="/v2/gateways/guides/monitoring-and-tooling/on-chain-debugging" label="On-Chain Debugging" newline={false} />. For Livepeer contract addresses, see <LinkArrow href="/v2/gateways/resources/technical/contract-addresses" label="Contract Addresses" newline={false} />.
    </BorderedBox>
</View>
<View title="CLI Tools" icon="terminal">
    <BorderedBox variant="accent">
        ...
        {/* REVIEW: Rick - confirm Option 1, 11, 16 and "Set broadcast config" are still correct in current builds */}
