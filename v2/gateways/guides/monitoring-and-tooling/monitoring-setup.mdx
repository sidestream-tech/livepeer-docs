...
This is the third of five monitoring guides. It sets up production observability for a Livepeer Gateway using Prometheus and Grafana. The Gateway must already be running - for quick verification, see <LinkArrow href="/v2/gateways/guides/monitoring-and-tooling/health-checks" label="Health Checks" newline={false} />.

For on-chain Gateways, Prometheus metrics complement but do not replace <LinkArrow href="/v2/gateways/guides/monitoring-and-tooling/on-chain-debugging" label="On-Chain Debugging" newline={false} /> - contract events and deposit state require separate monitoring.
...
<Tabs>
  <Tab title="Video" icon="video">
    <Badge color="blue">Video</Badge> - on-chain Gateways. Full reference: <LinkArrow href="/v2/gateways/resources/technical/go-livepeer/prometheus-metrics" label="Prometheus Metrics" newline={false} />.
    ...
    {/* REVIEW: Confirm livepeer_success_rate exists as a direct gauge, or whether it must be calculated from livepeer_segment_transcoded_total / livepeer_segment_source_appeared_total. j0sh to verify. */}
  ...
  <Tab title="AI" icon="brain">
    <Badge color="purple">AI</Badge> - both on-chain and off-chain Gateways. Fewer built-in metrics than video, but the ones that exist are critical.
  ...
  <Tab title="Dual" icon="clone">
    <Badge color="green">Dual</Badge> - on-chain Gateways running both video and AI. Run all video metrics AND all AI metrics. The additional concern is GPU resource contention.
...
{/* REVIEW: Rick - confirm whether the livepeer-monitoring repo dashboards cover AI metrics or are video-only. If video-only, note that AI dashboards must be created manually. */}
