...
This is the fourth and final advanced operations guide. Running a Gateway that routes internal traffic is one thing. Running a Gateway that other developers and applications can discover and use is another. This guide covers how to make a Gateway reachable and findable, and is transparent about what formal discoverability mechanisms exist today versus what is planned.

<Warning>
Unlike Orchestrators, which register on-chain via the ServiceRegistry contract and appear in the Livepeer Explorer automatically, there is no equivalent on-chain registry for Gateways. Discoverability today is manual and community-driven.
</Warning>
...
Point a dedicated subdomain at the Gateway server's public IP. A dedicated subdomain (rather than an IP address) allows changing servers without updating customer integrations.
...
<AccordionGroup>
  ...
  <Accordion title="Pricing" icon="tag">
    Publish customer-facing pricing clearly. The Gateway has two pricing layers:
...
<Note>
The following items are planned but not yet available. This section is speculative and will be updated as features reach production.
</Note>
...
<CardGroup cols={2}>
  ...
  <Card title="Opportunities" icon="lightbulb" href="/v2/gateways/guides/opportunities/overview">
