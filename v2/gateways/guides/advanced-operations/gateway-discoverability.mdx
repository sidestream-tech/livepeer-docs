...
<BorderedBox variant="accent">
    <Tabs>
        <Tab title="AI Registry" icon="brain">
            ...
            Use `-aiServiceRegistry` when the Gateway should discover AI Orchestrators from the open network rather than relying on a manually curated `-orchAddr` list. This is the on-chain equivalent of off-chain manual discovery.

            Without this flag, the Gateway operates off-chain and requires explicit Orchestrator addresses via `-orchAddr`.

            {/* REVIEW: Rick / Mehrdad - the AIServiceRegistry contract is noted as "detached from controller" in the contract addresses page. Confirm whether this is intentional architecture or a migration gap. */}
        </Tab>
        ...
        <Tab title="Video Registry" icon="video">
            ...
            {/* REVIEW: Confirm current roadmap status of on-chain transcoding gateway registry - Foundation to verify */}
        </Tab>
    </Tabs>
</BorderedBox>
