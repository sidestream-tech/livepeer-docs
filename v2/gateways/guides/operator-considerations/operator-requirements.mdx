...
A Livepeer gateway routes jobs between your application and the compute network. It does not run AI models or transcode video - orchestrator nodes handle that. Requirements vary by payment mode: off-chain (AI) gateways have a lighter setup, while on-chain (video) gateways need Ethereum infrastructure.
...
<CustomDivider style={{margin: "0 0 -1rem 0"}} middleText="Hardware" />
