...
description: 'Should you run a Livepeer gateway? The use case fit, business model, operator revenue patterns, and economics by deployment mode.'
keywords:
  - livepeer
...
<Tabs>
  ...
  <Tab title="Run your own gateway">
    ...
    **What it requires:**
    ...
    - No ETH required in off-chain deployment mode
    - On-chain deployment mode requires an ETH deposit on Arbitrum
  </Tab>
</Tabs>
...
<Note>
For AI gateways running in off-chain deployment mode, your gateway node itself holds no ETH. A remote signer handles all on-chain payment operations. Your gateway operator costs are whatever you pay the signer - which can be zero if using a community-hosted signer.
</Note>
...
<Tabs>
  <Tab title="Route your own workloads">
    ...
    **What this requires:**
    ...
    - No ETH for off-chain deployment mode; ETH deposit for on-chain deployment mode
  ...
  <Tab title="Embed in your platform">
    ...
    **What this requires:**
    - For AI: off-chain deployment mode, Linux, no ETH
...
The economic model differs between the two deployment modes.
...
<AccordionGroup>
  ...
  <Accordion title="Free to start with off-chain deployment mode">
    AI gateways in off-chain deployment mode require no ETH deposit and no Arbitrum wallet. You point at a community-hosted remote signer (which handles the payment layer for you), connect to orchestrators, and start routing inference requests immediately with no upfront crypto cost.

    This is a meaningful contrast to on-chain deployment mode, which requires bridging ETH to Arbitrum before you can process a single job.
  </Accordion>
</AccordionGroup>
...
<StyledTable variant="bordered">
  ...
  <tbody>
    ...
    <TableRow>
      ...
      <TableCell>Yes - off-chain deployment mode makes this low-friction</TableCell>
    </TableRow>
    <TableRow>
...
<CardGroup cols={3}>
  <Card title="Gateways in Practice" icon="building" href="/v2/gateways/guides/operator-considerations/gateway-showcase">
    Real products and community projects built on Livepeer gateways.
  </Card>
  <Card title="Requirements" icon="clipboard-check" href="/v2/gateways/guides/operator-considerations/operator-requirements">
    Hardware, OS, network, and deployment mode requirements.
  </Card>
  <Card title="Deployment Options" icon="arrow-right" href="/v2/gateways/guides/setup-options/deployment-options">
    Choose between AI, video, dual, platform, or SDK gateway paths.
  </Card>
</CardGroup>
