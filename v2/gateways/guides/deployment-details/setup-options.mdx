...
Gateway operators make three independent choices: **setup type** (what software to run), **operational mode** (on-chain or off-chain), and **node type** (what workloads to route). See the [Glossary](/v2/gateways/resources/glossary) for full definitions.

{/* TODO: This diagram sucks. Its taking up too much space. */}
```mermaid
%%{init: {'theme': 'dark', 'themeVariables': { 'primaryColor': '#1a1a1a', 'primaryTextColor': '#E0E4E0', 'primaryBorderColor': '#18794E', 'lineColor': '#2b9a66', 'secondaryColor': '#0d0d0d', 'tertiaryColor': '#3CB540', 'background': '#0d0d0d'}}}%%
flowchart TD
    subgraph ST ["Setup type"]
        direction LR
        B["go-livepeer\n(Docker or source)"]
        C["SDK / Custom\n(Python, browser, mobile)"]
        D["GWID\n(managed platform)"]
        E["Hosted\n(Livepeer Studio, Cloud)"]
    end

    subgraph OM ["Operational mode"]
        direction LR
        G["Off-chain\n(remote signer)"]
        H["On-chain\n(Arbitrum)"]
    end

    subgraph NT ["Node type"]
        direction LR
        J["Video"]
        K["AI"]
        L["Dual"]
...
<Tabs>
  ...
  <Tab title="SDK / Custom" icon="code">
    ...
    **Workload support:**
    ...
    {/* REVIEW: Confirm whether batch AI remote signer support has shipped. Check go-livepeer PRs #3791 and #3822. */}
  ...
  <Tab title="GWID" icon="wand-magic-sparkles">
  ...
  <Tab title="Hosted Gateway" icon="cloud">
    ...
    **Hosted gateway providers:**
    ...
    <Card title="Gateway Providers" icon="merge" href="/v2/developers/navigator" arrow horizontal>
