...
{/* REVIEW: Verify FrameProcessor is the current class name in pytrickle. The repo may use process_video_async() instead. */}
...
<StyledSteps iconColor="#2d9a67" titleColor="var(--accent)">
  ...
  <StyledStep title="Test in isolation" icon="flask-vial">
    ...
    {/* REVIEW: The http-trickle package (j0sh/http-trickle) is a Go project, NOT pip-installable. This testing step needs verification. The container may need to be tested via direct HTTP calls instead. */}
  ...
  <StyledStep title="Optional: Whisper-tiny inference" icon="microphone">
    Once the green-tint passthrough works, swap the processor for a real AI model. Whisper-tiny (79M parameters) runs on CPU.
