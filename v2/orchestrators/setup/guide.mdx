---
title: Run an Orchestrator
sidebarTitle: Setup Overview
description: >-
  End-to-end setup path for Livepeer orchestrators running go-livepeer —
  from hardware prerequisites through activation and first jobs.
keywords:
  - livepeer
  - orchestrator
  - setup
  - go-livepeer
  - activation
  - GPU
  - install
  - configure
'og:image': /snippets/assets/site/og-image/en/orchestrators.png
'og:image:alt': Livepeer Docs social preview image for Orchestrators
'og:image:type': image/png
'og:image:width': 1200
'og:image:height': 630
pageType: overview
audience: orchestrator
purpose: guide
status: current
lastVerified: 2026-03-13
---

This is the standard setup path for running a Livepeer orchestrator with go-livepeer. It covers a single-machine deployment where one node runs as both orchestrator and transcoder.

```mermaid
%%{init: {'theme': 'base', 'themeVariables': { 'primaryColor': '#1a1a1a', 'primaryTextColor': '#fff', 'primaryBorderColor': '#2d9a67', 'lineColor': '#2d9a67', 'secondaryColor': '#0d0d0d', 'tertiaryColor': '#1a1a1a', 'background': '#0d0d0d' }}}%%
flowchart LR
    classDef default stroke-width:2px
    GPU["Your Machine\n(GPU + go-livepeer)"]
    Arb["Arbitrum One\n(staking, payments)"]
    GW["Gateways\n(route jobs to you)"]
    GPU -->|"register, stake, reward"| Arb
    GW -->|"send video/AI jobs"| GPU
    GPU -->|"return results"| GW
```

<Note>
**Not the right path?** If you want to contribute GPU to an existing pool without managing your own node, see [Join a Pool](/v2/orchestrators/guides/setup-paths/join-a-pool). For the split keystore setup (Siphon), see [Siphon Setup](/v2/orchestrators/guides/setup-paths/siphon-setup). For help choosing, see the [Navigator](/v2/orchestrators/guides/setup-paths/setup-navigator).
</Note>

---

## Setup flow

Follow these steps in order. Each page is self-contained — complete it before moving to the next.

<Steps>
  <Step title="Check prerequisites">
    Confirm your hardware, GPU drivers, network, wallet, and tokens are ready.

    [Setup Checklist →](/v2/orchestrators/setup/rcs-requirements)
  </Step>
  <Step title="Install go-livepeer">
    Download and install the go-livepeer binary or Docker image for your platform.

    [Install →](/v2/orchestrators/setup/rs-install)
  </Step>
  <Step title="Configure your node">
    Set the startup flags: network, GPU selection, pricing, service address, and AI workers.

    [Configure →](/v2/orchestrators/setup/r-configure)
  </Step>
  <Step title="Connect to Arbitrum">
    Choose an RPC provider, add the `-ethUrl` flag, and verify the connection.

    [Connect →](/v2/orchestrators/setup/sc-connect)
  </Step>
  <Step title="Activate on-chain">
    Register as an orchestrator, set reward cut and fee cut, stake LPT, and join the active set.

    [Activate →](/v2/orchestrators/setup/activate)
  </Step>
  <Step title="Verify your setup">
    Run pre- and post-activation checks: GPU detection, port reachability, Explorer status, first job.

    [Verify →](/v2/orchestrators/setup/test)
  </Step>
  <Step title="Set up monitoring">
    Enable Prometheus metrics, track key health signals, and connect to Explorer.

    [Monitor →](/v2/orchestrators/setup/r-monitor)
  </Step>
</Steps>

---

## What you will need

| Requirement | Details |
|---|---|
| **GPU** | NVIDIA with NVENC support. RTX 3060+ for transcoding; RTX 3090+ for AI inference. |
| **OS** | Linux recommended for production. macOS and Windows for development only. |
| **CUDA** | 12.0+ with matching NVIDIA driver (525+ on Linux). |
| **Wallet** | Ethereum account with ETH on Arbitrum (for gas) and LPT (for staking). |
| **Port** | 8935 TCP open to the public internet. |
| **RPC** | Arbitrum One endpoint (Alchemy, Infura, or self-hosted). |

For detailed hardware guidance, see [GPU & Hardware Reference](/v2/orchestrators/guides/feasibility-and-hardware/hardware-reference). For cost and revenue analysis, see [Feasibility & Economics](/v2/orchestrators/guides/feasibility-and-hardware/feasibility-economics).

---

## After setup

Once your node is live and receiving jobs:

<CardGroup cols={2}>
  <Card title="Workloads & AI" icon="layer-group" href="/v2/orchestrators/guides/workloads-and-ai/ai-workloads-guide">
    Add AI inference pipelines to earn from the AI subnet alongside transcoding.
  </Card>
  <Card title="Staking & Rewards" icon="coins" href="/v2/orchestrators/guides/staking-and-rewards/earnings">
    Understand your revenue streams and optimise commission parameters.
  </Card>
  <Card title="Getting Delegates" icon="users" href="/v2/orchestrators/guides/staking-and-rewards/attracting-delegates">
    Attract delegators to increase your stake and job selection probability.
  </Card>
  <Card title="Troubleshooting" icon="circle-question" href="/v2/orchestrators/resources/faq">
    Common errors and fixes for orchestrator operations.
  </Card>
</CardGroup>
