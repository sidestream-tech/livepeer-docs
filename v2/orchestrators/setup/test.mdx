...
This is the final setup step. It confirms the node is on-chain, processing jobs correctly, and exposing the minimum monitoring signals. Full monitoring setup (Grafana, alerting, DCGM exporter) is in <LinkArrow href="/v2/orchestrators/guides/monitoring-and-tooling/metrics-and-alerting" label="Guides > Monitoring and Tools" newline={false} />.
...
<StyledSteps>
  ...
  <StyledStep title="Test AI inference (AI and dual mode)">
    ...
    {/* REVIEW: Confirm /text-to-image is the correct direct orchestrator endpoint for AI inference in current go-livepeer. j0sh to verify. */}
  ...
  <StyledStep title="Confirm reward calling is active">
    ...
    Confirm `-reward=false` is **not** present. Its absence means automatic calling is enabled.
    ...
    {/* REVIEW: Confirm the exact log message string for a successful reward call in current go-livepeer. Rick to verify "Calling reward" or similar. */}

    If reward calls are missing after a round: confirm the node was online and connected during round initialisation, that the Arbitrum ETH balance is sufficient for gas, and that `-reward=false` is not in the startup command.
...
<AccordionGroup>
  ...
  <Accordion title="Not appearing on Explorer" icon="magnifying-glass">
    Confirm the activation transaction succeeded on Arbiscan. Check the service URI is set and reachable. Verify the stake bonding transactions (both `approve` and `bond`) confirmed. If recently activated, wait for the next round - the active set updates once per round (~22 hours).
  </Accordion>
  <Accordion title="Reward calls not appearing" icon="gift">
    Confirm `-reward=false` is not in the startup command. Check the Arbitrum ETH balance - reward calls require gas. Confirm the node was online during round initialisation. Check logs for reward call errors. See <LinkArrow href="/v2/orchestrators/guides/config-and-optimisation/reward-call-tuning" label="Reward Call Tuning" newline={false} /> for low-stake profitability analysis.
  </Accordion>
  <Accordion title="Not receiving jobs after 48 hours" icon="clock">
...
<Note>
  The node is now on-chain, processing jobs, and verified. The setup section is complete. Earning from the node - pricing optimisation, delegation strategy, workload selection, and monitoring - is covered in the Guides section.
</Note>
