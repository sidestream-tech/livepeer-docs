...
- go-livepeer is running and connected to Arbitrum One (see [Connect to Arbitrum](/v2/orchestrators/setup/sc-connect))
...
<Steps>
  ...
  <Step title="Set price per unit">
    ...
    <Note>
    Setting your price too high relative to other orchestrators means gateways will route jobs elsewhere. Check current network pricing on [Livepeer Explorer](https://explorer.livepeer.org) or [livepeer.tools](https://livepeer.tools) to set a competitive rate.
    </Note>
  </Step>
...
<Note>
After your activation transactions confirm, it may take until the next round begins before your status changes to Active in Explorer. Livepeer rounds are approximately 24 hours. {/* REVIEW: confirm current round length with protocol team */}
</Note>
...
For further troubleshooting, see [FAQ](/v2/orchestrators/resources/faq) or ask in `#orchestrating` on [Discord](https://discord.gg/livepeer).
</Accordion>
...
<CardGroup cols={3}>
  <Card title="Verify Your Setup" icon="circle-check" href="/v2/orchestrators/setup/test" arrow />
