...
<Note>
  This page verifies hardware. For production setup with on-chain activation and earning, continue to the <LinkArrow href="/v2/orchestrators/setup/guide" label="Setup Guide" newline={false} /> after completing these tests.
</Note>
...
<StyledTable variant="bordered">
  ...
  <tbody>
    ...
    <TableRow>
      ...
      <TableCell>If this fails, install the toolkit before continuing.</TableCell>
    </TableRow>
    <TableRow>
...
<StyledSteps>
  ...
  <StyledStep title="Send a test stream">
    ...
    Send a synthetic test pattern as an RTMP stream. This simulates real video from OBS or a camera:
...
The gateway received the RTMP stream, split it into segments, routed them to the orchestrator, the orchestrator transcoded each segment on the GPU, and the gateway reassembled the output as an HLS stream. No Arbitrum, no ETH, no LPT - the `-network offchain` flag bypassed all blockchain interaction.
...
<StyledSteps>
  ...
  <StyledStep title="Test inference locally">
    ...
    {/* REVIEW: Confirm exact local inference endpoint path. j0sh / Rick to verify /text-to-image is the correct direct orchestrator endpoint in current go-livepeer. */}
