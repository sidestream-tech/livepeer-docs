...
<Tip>
  ETH earnings do not accumulate every minute. They arrive in discrete steps whenever a winning ticket is redeemed on-chain. A node that processed thousands of jobs may have zero ETH balance changes for hours, then receive a larger deposit. This is expected behaviour, not a payment failure.
</Tip>

---

Gateways pay orchestrators using **probabilistic micropayments (PM)**: a system where each job generates a signed ticket with a small probability of being worth a larger ETH payout. Most tickets are non-winners and are discarded. A fraction win and are redeemed on-chain via the Arbitrum TicketBroker contract. Over sufficient volume, the expected ETH received matches what would have been paid per-segment with individual transactions — without the gas overhead of thousands of individual on-chain payments.

<CustomDivider middleText="How Tickets Arrive" />

## How tickets arrive

A gateway attaches one PM ticket to each job confirmation it sends after receiving the transcoded output or AI inference result. The ticket is received passively — your node does not request it or trigger it. The flow is:
...
<StyledTable variant="bordered">
  ...
  <tbody>
    <TableRow>
      ...
      <TableCell>ETH amount payable if this ticket wins</TableCell>
    </TableRow>
    <TableRow>
...
Over a large number of tickets, the ETH received converges to the amount you would have been paid if every job had its own transaction. At low job volumes, the variance is high — a node processing 10 jobs per day might see zero redemptions some days and two on others. Variance decreases as volume increases.
...
{/* REVIEW: Confirm gas cost figures against current Arbitrum gas prices. The $0.01-$0.05 range is from payments.mdx (March 2026). Arbitrum gas prices have varied; verify current typical range with Rick. */}

The Redeemer batches pending winners when possible. A busy orchestrator redeems ETH a few times per day rather than once per winning ticket, keeping gas overhead negligible relative to earnings.
...
<Warning>
  Winning tickets carry an expiry block number. If your wallet runs out of ETH before submission, expired tickets cannot be retroactively redeemed. The gateway's deposit is unaffected; the loss falls entirely on your side.
</Warning>
...
<StyledTable variant="bordered">
  ...
  <tbody>
    <TableRow>
      ...
      <TableCell>A ticket passed the win condition; queued for Redeemer</TableCell>
    </TableRow>
    <TableRow>
    ...
    <TableRow>
      ...
      <TableCell>Winning ticket not submitted before expiry block; ETH permanently lost</TableCell>
    </TableRow>
  </tbody>
</StyledTable>

{/* REVIEW: Confirm exact log message strings against current go-livepeer source. The strings above are representative; Rick to verify exact wording in current release. */}
...
- **ETH balance depleted** — winning tickets fail to redeem silently if gas cannot be paid.
- **RPC connectivity issue** — check your `-ethUrl` endpoint is reachable and not rate-limited.
...
This page covers the receiver-side mechanics of PM tickets: how they arrive, win conditions, and redemption. The <LinkArrow href="/v2/orchestrators/guides/payments-and-pricing/payments" label="Payments" newline={false} /> page covers broader payment flow concepts including the full PM system architecture, ETH funding requirements across all transaction types, video vs AI payment differences, and clearinghouses. Read both for the complete picture.
