...
<Accordion title="OrchestratorCapped — node not accepting work">
...
4. If you are running a split setup, verify the transcoder machine (not just the orchestrator) has capacity headroom

See [Session Limits](/v2/orchestrators/guides/feasibility-and-hardware/session-limits) for the full calculation methodology.
</Accordion>

<Accordion title="Transcode loop timed out / Segment loop timed out">
...
<Accordion title="MB rate > Level limit warning">
...
**In practice:** This warning usually does not prevent transcoding from completing. As long as transcoding finishes and results are returned to the gateway, your node is operating normally. If you are seeing a high rate of these warnings combined with transcoding failures, check the source segment properties — but this is a gateway or broadcaster issue, not an orchestrator issue.
</Accordion>

<Accordion title="Unable to transcode errors">
...
<Accordion title="Cannot allocate memory (on startup with -nvidia flag)">
...
{/* REVIEW: Confirm whether the Ada Lovelace (RTX 40xx) generation removed the NVENC session cap, or whether patching is still required. Rick to verify current driver state. */}
</Accordion>

<Accordion title="CUDA out of memory (AI inference)">
...
See [Model VRAM Reference](/v2/orchestrators/guides/feasibility-and-hardware/hardware#ai-vram-requirements) for per-pipeline minimums.
</Accordion>

<Accordion title="Node not using GPU for transcoding despite -nvidia flag">
...
<Accordion title="Node still calling reward despite -reward=false">
...
4. If using a `.conf` file for configuration, the command-line flag overrides the file — always pass `-reward=false` explicitly at launch, not just in the config file.
</Accordion>

<Accordion title="Missing reward rounds despite -reward=true">
...
2. **Node was offline** — if your node was down at the round boundary (~every 22 hours on Arbitrum), the call was not made. Check your systemd/service uptime logs.
3. **Multiple processes competing** — if you have two go-livepeer processes with the same wallet, one may be submitting a failing duplicate.

For persistent missed rounds, consider the [Siphon split setup](/v2/orchestrators/guides/setup-paths/siphon-setup) which runs reward calling on a dedicated stable machine independently of your GPU workload.
</Accordion>

<Accordion title="TicketParams expired">
...
|---|---|---|
| Wrong Docker image tag | `docker logs livepeer-ai-runner` shows image pull error | Use the correct versioned tag from `aiModels.json` documentation |
| VRAM insufficient | `docker logs livepeer-ai-runner` shows CUDA OOM at startup | Reduce loaded models, free VRAM, or check minimum requirements |
| NVIDIA Container Toolkit missing | `docker run --gpus all nvidia/cuda:11.0-base nvidia-smi` fails | Install `nvidia-container-toolkit` — see [Hardware Reference](/v2/orchestrators/guides/feasibility-and-hardware/hardware#nvidia-container-toolkit) |
| Docker not in GPU mode | Container starts but no GPU visible inside | Pass `--gpus all` or `--gpus device=<id>` in docker run |
</Accordion>

<Accordion title="Wrong model ID — model fails to load">
**What it means:** The model ID in your `aiModels.json` does not resolve to a real model.
...
<Accordion title="AI pipeline registered but receiving no jobs">
**Diagnose in order:**

1. **Not in active set** — check [Livepeer Explorer](https://explorer.livepeer.org/orchestrators). If you are not in the top 100 by stake, you receive no transcoding or AI jobs.
...
3. **Model is cold** — if the warm model is not loaded in VRAM, jobs may time out before the model loads (cold start can take 30 seconds to several minutes). Make sure your intended warm model is listed in the `warm` section of `aiModels.json`.
...
<Accordion title="OOM during AI inference (job starts, then fails)">
...
{/* REVIEW: Confirm whether go-livepeer/ai-runner supports per-pipeline dimension limits, and whether orchestrators can reject oversize requests. Rick / Peter to confirm. */}
</Accordion>

<Accordion title="ComfyStream container failing (real-time AI)">
**Checklist:**
...
3. The container has sufficient VRAM for the workflow — real-time streaming requires the model to remain resident during the stream
4. Check container logs for the specific Python exception: `docker logs -f <comfystream-container>`

See [Real-Time AI Setup](/v2/orchestrators/setup/realtime-ai-setup#troubleshooting) for ComfyStream-specific troubleshooting.
</Accordion>

<Accordion title="LLM pipeline (Ollama) not receiving jobs">
**Checklist:**
1. Verify Ollama container is running and accessible: `curl http://localhost:11434/api/version`
2. Confirm go-livepeer can reach the Ollama endpoint (Docker networking — if both are in containers, they may need to be on the same Docker network)
...
<Accordion title="Service address mismatch warning at startup">
**Error:**
```
...
<Accordion title="Node not receiving any jobs despite being in active set">
...
<Accordion title="Running behind a NAT or home router">
...
<Accordion title="What is the service URI — can it be a hostname?">
...
<Accordion title="Error creating Ethereum account manager">
...
<CardGroup cols={2}>
  ...
  <Card title="Explorer Guide" icon="compass" href="/v2/orchestrators/guides/monitoring/explorer">
