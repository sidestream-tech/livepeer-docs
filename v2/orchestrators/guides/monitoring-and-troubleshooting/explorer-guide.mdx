---
title: Reading the Livepeer Explorer
...
The Livepeer Explorer reads directly from the Arbitrum subgraph. Every number here reflects on-chain state. This guide walks through your orchestrator profile page metric by metric — what it shows, what healthy looks like, and what to do when it does not.
...
## What the Explorer does and does not show
...
- Real-time node health or uptime (use Prometheus for this)
...
**If you are not in the active set:**
...
{/* REVIEW: Confirm "top 100" is still the current active set size. Appears consistently in project files but verify against current protocol parameter. Mehrdad/Rick to confirm. */}
...
<Accordion title="Self-stake vs delegated stake">
Your self-stake is the LPT you bonded to yourself. Delegated stake comes from LPT holders who chose to delegate to you.

You can increase your self-stake directly by bonding more LPT. Attracting delegators requires competitive reward cut and fee share settings and a strong performance track record — see [Getting Delegates](/v2/orchestrators/guides/staking-and-rewards/getting-delegates).
</Accordion>

<Accordion title="How stake affects job routing">
...
<Accordion title="Pending stake vs current stake">
...
| Parameter | What it means | Who it affects |
|---|---|---|
| **Reward cut** | Percentage of LPT inflation you keep. The rest goes to delegators proportional to their stake. | Your income + delegator attraction |
| **Fee share** | Percentage of ETH transcoding fees you share with delegators. | Delegator yield |
...
<Warning>
  Changes to reward cut and fee share are publicly visible to delegators and affect their yield. Frequent large changes can reduce delegator confidence. Communicate changes in advance if you have a significant delegating base.
</Warning>
...
<Accordion title="Check if -reward=false is set accidentally">
If you disabled reward calling intentionally, `-reward=false` is the flag. If you are using a split setup where the orchestrator and transcoder run as separate processes, ensure `-reward=false` (or the absence of this flag, letting it default to true) is consistent across all launch commands. A common mistake is setting `-reward=false` on the orchestrator but a separate process still running the wallet.
</Accordion>

<Accordion title="Check your ETH balance for gas">
Reward calls require ETH on Arbitrum for gas. If your wallet runs dry, reward calls will fail silently. Check your orchestrator address ETH balance on Arbiscan. Keep at least 0.02–0.05 ETH in the wallet as a buffer.
</Accordion>

<Accordion title="Check node uptime at round boundaries">
If your node was offline at the round boundary, the reward call was not made. The Arbitrum round length is approximately 22 hours. If your node has any regular maintenance windows, check whether these coincide with round boundaries.
</Accordion>

<Accordion title="Using OrchestratorSiphon">
If missed rewards are a recurrent problem, consider the [split setup](/v2/orchestrators/guides/setup-paths/siphon-setup). OrchestratorSiphon runs reward calling on a separate, stable machine independently of your GPU workload machine.
</Accordion>
...
- Large single-delegator dependence — if one delegator accounts for most of your stake, their departure would significantly affect your active set position.
...
|---|---|---|
| Active set rank dropping week-over-week | Relative stake declining vs competitors | Attract delegators or increase self-stake |
| Missed reward rounds | Reward calling failing | Check ETH balance, `-reward` flag, consider Siphon |
| Zero fee earnings despite active status | Not receiving work | Check pricing, reachability, capabilities |
| Delegator count falling | Delegators unbonding | Check reward call rate, review reward cut competitiveness |
| Service URI mismatch warning at startup | On-chain URI doesn't match node IP | Update `-serviceAddr` or update on-chain URI |

---

<CardGroup cols={2}>
  <Card title="Metrics and Monitoring" icon="gauge" href="/v2/orchestrators/guides/monitoring/metrics">
    Set up Prometheus and Grafana for real-time node health monitoring.
  </Card>
  <Card title="Getting Delegates" icon="users" href="/v2/orchestrators/guides/staking-and-rewards/getting-delegates">
