...
This guide covers the operator-side configuration for video transcoding — how to set your price, how to calculate a safe session limit, how NVENC hardware caps affect concurrency, and what the standard output rendition profiles look like. For hardware benchmarking, see the dedicated [Benchmarking Guide](/v2/orchestrators/guides/feasibility-and-hardware/benchmarking).
...
{/* REVIEW: Confirm minimum NVIDIA driver version for current go-livepeer release. From gpu.md: tested with CUDA 10.0–11.x; note that 450.xx drivers have a known stuck-session issue. Rick to confirm current minimum recommendation. */}
...
| Base price | Redemption overhead | Advertised price |
|---|---|---|
| 1000 wei/px | 1% | 1010 wei/px |
| 1000 wei/px | 20% | 1200 wei/px |
| 1000 wei/px | 50% | 1500 wei/px |
...
You can update your price without restarting the node:
...
<Step title="Select Set orchestrator config">
...
Within the pool, lower prices increase your selection probability (gateways weight price among other factors). Being the cheapest does not guarantee the most work, but being above the ceiling guarantees none.
...
{/* REVIEW: livepeer.tools was referenced in v1 docs as a market rate tool but the domain is down. tools.livepeer.cloud is the current community equivalent. Confirm with community before publication. */}
...
For the full benchmarking walkthrough including test stream download and CSV output analysis, see [Benchmarking](/v2/orchestrators/guides/feasibility-and-hardware/benchmarking).

### Calculating bandwidth capacity

The standard ABR output ladder consumes predictable bandwidth per session:

| Direction | Per session | Basis |
|---|---|---|
| **Download** (inbound source) | ~6 Mbps | 1080p30fps source at ~6000 kbps |
| **Upload** (outbound output) | ~5.6 Mbps | Full 5-rendition ABR ladder total |
...
This is a starting point, not a permanent answer. Monitor your Real-Time Duration Ratio in production (via Prometheus metrics) and back off if it consistently exceeds 0.8 under load.
...
| GPU tier | NVENC session cap |
|---|---|
| Consumer (GTX 10xx, RTX 20xx–40xx) | 3–8 concurrent sessions per GPU |
| Professional (RTX A-series, Quadro) | Unlimited |
| Data-centre (A100, H100) | Unlimited |
...
{/* REVIEW: Ada Lovelace (RTX 40xx) generation — confirm whether the NVENC session cap applies or was removed. Rick to verify before publication. */}
...
|---|---|---|
| P720p30fps | 720p at source fps | 3000 kbps |
| P480p30fps | 480p at source fps | 1600 kbps |
| P360p30fps | 360p at source fps | 800 kbps |
| P240p30fps | 240p at source fps | 250 kbps |

{/* REVIEW: Confirm whether 1080p has been removed from the default transcodingOptions.json or whether it varies per gateway. Rick confirmed 1080p removed from transcodingOptions.json in session 3 — but gateways may still request it in source passthrough. Confirm before publication. */}
...
<Accordion title="Always use GPU transcoding">
...
<Accordion title="Derive maxSessions from benchmarks, not guesses">
...
<Accordion title="Monitor Real-Time Duration Ratio in production">
Set up Prometheus metrics (`-monitor` flag) and watch the Duration Ratio under real load. If it climbs above 0.8 consistently during peak periods, lower `-maxSessions`. The benchmark is an approximation — real-world stream properties vary and the production value may differ.
</Accordion>

<Accordion title="Use a domain name for serviceAddr, not a bare IP">
...
<Accordion title="USD pricing for long-term stability">
...
<Accordion title="Leave autoAdjustPrice on during gas price spikes">
...
<CardGroup cols={2}>
  <Card title="Benchmarking Guide" icon="gauge-high" href="/v2/orchestrators/guides/feasibility-and-hardware/benchmarking">
    Full livepeer_bench walkthrough — test stream download, concurrent session sweep, CSV output analysis.
  </Card>
  <Card title="Session Limits" icon="sliders" href="/v2/orchestrators/guides/feasibility-and-hardware/session-limits">
    Detailed bandwidth and hardware capacity methodology with worked examples.
  </Card>
  <Card title="Job Types Overview" icon="list" href="/v2/orchestrators/guides/workloads-and-ai/job-types-overview">
