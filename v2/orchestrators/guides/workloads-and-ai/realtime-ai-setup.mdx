---
title: Real-time AI Setup
sidebarTitle: Real-time AI Setup
description: >-
  Deploy the live-video-to-video pipeline on a Livepeer orchestrator — Cascade
  architecture, ComfyStream setup, real-time frame processing, WebRTC streaming,
...
Real-time AI inference is fundamentally different from batch inference. Instead of processing a discrete request and returning a result, your node continuously transforms live video — receiving frames from a WebRTC stream, running inference on each frame, and streaming processed frames back with sub-100ms latency.

This is the **Cascade** architecture: the `live-video-to-video` pipeline that powers live AI video effects, real-time style transfer, and streaming AI agents on the Livepeer network.

---

## What Cascade is

Cascade is Livepeer's real-time AI video processing pipeline. The name refers to the architecture — video streams cascade through AI transformation nodes in the network, enabling live applications that were previously impossible without centralised infrastructure.

Real-world applications running on Cascade:

- **Daydream** — generative AI video platform with real-time style application
...
|---|---|---|
| **Input** | Discrete file or prompt | Continuous WebRTC stream |
| **Output** | Result returned | Processed WebRTC stream |
| **Latency target** | Seconds per request | &lt;100ms per frame |
| **Inference pattern** | Request → process → respond | Continuous frame loop |
| **Runtime** | `livepeer/ai-runner` | `livepeer/ai-runner:live-base` |
| **Pipeline interface** | REST API | `Pipeline` interface (async frame queue) |
| **Models** | Standard diffusion, Whisper, BLIP | StreamDiffusion, ComfyUI DAGs, ControlNet |
| **Min VRAM** | 4–24 GB | 24 GB recommended |
...
Real-time AI has stricter hardware requirements than batch:
...
<Warning>
  GPUs below 24 GB VRAM (e.g. RTX 3080 10 GB, RTX 3060 12 GB) are typically insufficient for real-time AI inference at acceptable quality and frame rates. The combination of model weights, StreamDiffusion/ComfyUI overhead, and frame buffers exhausts available VRAM.
</Warning>
...
## ComfyStream — the real-time pipeline runtime

[ComfyStream](https://github.com/livepeer/comfystream) is the primary runtime for real-time AI inference on Livepeer. It wraps ComfyUI's node-based workflow system and adapts it for continuous frame processing.
...
- StreamDiffusion (optimised for real-time diffusion at 30+ fps)
...
**Source:** [livepeer/comfystream on GitHub](https://github.com/livepeer/comfystream) · [ComfyUI-Stream-Pack](https://github.com/livepeer/ComfyUI-Stream-Pack) {/* REVIEW: Rick must confirm canonical node locations — ComfyUI-Stream-Pack README states no nodes have been added */}
...
<Step title="Verify GPU and driver setup">
```bash
nvidia-smi
...
<Step title="Pull the live-base AI runner image">
```bash
...
{/* REVIEW: Verify current image tag with Rick / go-livepeer Docker image map */}
</Step>

<Step title="Configure aiModels.json for the live pipeline">
```json
[
  {
...
`model_id` for real-time pipelines refers to the ComfyUI workflow or pipeline name, not a HuggingFace model ID in the traditional sense. The models themselves are loaded inside the ComfyStream container.

{/* REVIEW: Confirm exact model_id format for live-video-to-video with Rick/j0sh — format may differ from batch pipelines */}

**Pricing and gateway compatibility:** Your `price_per_unit` must be at or below the gateway's configured `-maxPricePerCapability` for `live-video-to-video`. This is a hard filter — gateways will not route jobs to your node if your price exceeds their cap, regardless of your hardware quality or latency.

For transcoding, the equivalent flag is `-maxPricePerUnit` on the gateway side. Before going live, verify that your transcoding price (set via `-pricePerUnit` or `livepeer_cli`) is below what major gateways have configured as their maximum. See [Gateway Pricing Configuration](/v2/gateways/run-a-gateway/pricing-configuration) for the full gateway pricing flag reference and the JSON structure used for AI capability pricing.
</Step>

<Step title="Download the pipeline model weights">
...
<Step title="Start go-livepeer with live AI flags">
```bash
docker run -d \
  --gpus all \
...
<Step title="Verify the pipeline is registered">
...
You can also test the pipeline locally by sending a test WebRTC stream to your node. For setup scripts and test utilities, see the [ComfyStream repository](https://github.com/livepeer/comfystream).
</Step>
...
## StreamDiffusion — real-time diffusion models

[StreamDiffusion](https://github.com/cumulo-autumn/StreamDiffusion) is the primary model architecture for real-time video AI on Livepeer. It was designed specifically for continuous frame processing and achieves 30+ fps on an RTX 4090.

**How StreamDiffusion achieves real-time performance:**
...
A typical ComfyStream workflow for real-time style application:
...
Steps as low as `2` enable real-time performance. Quality vs latency is tunable through the workflow.

---

## The Pipeline interface (custom pipelines)

For operators who want to build custom real-time AI processing beyond ComfyUI, the AI runner exposes a Python `Pipeline` interface. Custom pipelines extend this interface and are packaged as Docker images extending `livepeer/ai-runner:live-base`.

```python
from runner.live.pipelines import Pipeline
from runner.live.trickle import VideoFrame, VideoOutput

class MyRealtimePipeline(Pipeline):
...
Custom real-time pipelines require two code changes in the upstream repositories:
...
These changes are required because the pipeline registry is not fully dynamic yet. See the full custom pipeline development guide in the [ai-runner.md](/mnt/project/ai-runner.md) documentation.
...
## Model types for real-time inference

### StreamDiffusion (primary)

Best for: Continuous style application, generative video effects, real-time prompt-to-video
...
| ControlNet | Input signal | Use case |
|---|---|---|
| Depth (MiDaS / DepthAnything) | Depth map | Preserve 3D structure while changing appearance |
| Canny / Sketch | Edge map | Structural outline preservation |
| Pose (DWPose) | Pose keypoints | Character animation, avatar driving |
| Normal | Surface normals | Lighting-aware transformation |
...
Real-time AI performance is dominated by inference latency per frame. Key levers:
...
**Stochastic Similarity Filter:** Enable in StreamDiffusion to skip inference on static or slow-moving frames. Significant fps improvement with no visible quality loss for typical video content.

### VRAM management

Unlike batch AI, real-time pipelines hold models in VRAM continuously for the duration of the stream. VRAM must be reserved for:
...
A single RTX 4090 can typically handle **1–2 concurrent real-time streams** depending on resolution and model complexity. For multi-stream capacity:

- **Multiple GPUs:** The AI worker can dispatch streams across multiple physical GPUs
- **Model parallelism:** Not natively supported in ComfyStream (single-GPU per stream)
- **Scale-out:** Run multiple orchestrator instances, each handling 1–2 streams, behind a gateway load balancer

Enterprise operators running 4090/A100 fleets can monetise real-time AI at scale by advertising higher `capacity` values and maintaining multiple warm pipeline instances.

---

## Watch: Real-time AI and Cascade
...
<Frame>
  <iframe
    ...
    title="ComfyStream — Real-time AI Video with Livepeer"
    frameBorder="0"
...
<CardGroup cols={2}>
  ...
  >
    Full Q1 2025 bootcamp session. Covers ComfyStream, Cascade architecture, and orchestrator setup for real-time inference.
  </Card>
  <Card
  ...
  >
    StreamDiffusion GitHub repository includes benchmark videos showing 30+ fps real-time style transfer.
  </Card>
</CardGroup>
...
<Accordion title="Frames dropping or high latency">
...
<Accordion title="Pipeline not receiving real-time jobs">
...
<Accordion title="ComfyStream container failing to start">
...
<Accordion title="Custom pipeline not receiving jobs after registration">
...
<CardGroup cols={2}>
  ...
  <Card title="AI Workloads Overview" icon="diagram-project" href="/v2/orchestrators/concepts/ai-workloads">
    Batch vs real-time, pipeline types, and network routing.
  </Card>
</CardGroup>
