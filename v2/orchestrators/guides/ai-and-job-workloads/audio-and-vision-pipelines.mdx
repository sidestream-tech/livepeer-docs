...
**Competitive note:** Whisper is VRAM-efficient. A 12 GB or 24 GB card can hold Whisper warm alongside a diffusion model warm on a separate GPU.
...
{/* REVIEW: Verify the per-millisecond pricing unit for audio-to-text is correctly implemented in go-livepeer — confirm it is milliseconds and not another audio time unit. j0sh to verify. */}
...
{/* REVIEW: Confirm current network-preferred TTS model and whether the pricing unit is per character or per ms — the source docs show "per character or per ms of output audio" which suggests it varies. j0sh to clarify the actual go-livepeer implementation. */}
...
**Entry point:** Runs on 4 GB GPUs. An operator with a GPU below the diffusion threshold (24 GB) can contribute via `image-to-text` and `audio-to-text`.
...
`segment-anything-2` can be left cold (no `"warm": true`) unless demand justifies the VRAM cost. The model loads on first request.

{/* REVIEW: Confirm SAM2 model_id format — source shows "facebook/sam2-hiera-large" but the original Meta repository uses "facebookresearch/segment-anything-2". Verify the correct HuggingFace path. Rick / ai-runner team to confirm. */}
...
Audio and vision pipelines can run alongside diffusion pipelines on the same node. Example configuration for a 24 GB card with diffusion warm and Whisper also warm:
...
<Warning>
  During the Beta phase, only one warm model per GPU is supported. If `text-to-image` and `audio-to-text` are on the same physical GPU, only one can be warm. Assign them to separate GPUs, or keep one cold. Check logs for `Error loading warm model` at startup.
</Warning>

{/* REVIEW: Confirm whether the one-warm-model-per-GPU Beta constraint applies per-GPU or per-container. If audio-to-text runs in its own container on its own GPU, this constraint may not apply cross-pipeline. Rick / ai-runner team to clarify. */}
