...
Download time varies by model size and network speed. A large diffusion model that spans multiple gigabytes may take a few minutes to download on the first run. The container waits until the model is ready before serving requests.
