...
Running a Livepeer Orchestrator is only worth it when your hardware, stake access, and operating
tolerance line up with a realistic way to earn. For some readers that means keeping the commitment
low and joining a pool. For others it means committing to solo video, AI-first operation, or a more
commercial path.

This page is the evaluation layer for that decision. It breaks the choice into costs, revenue
sources, and a small set of viability checks so you can decide whether to stop, start small, or
commit to a full Orchestrator path.
...
Neither stream pays out simply because the software is running. ETH fees depend on Gateways deciding
to send work. LPT rewards depend on stake, activation, and a node that calls `Reward()` reliably.
...
Revenue projections are only useful when the cost baseline is honest. The real question is not just
whether a node can earn, but whether the earning path fits what you already own and what you are
willing to keep paying for.
...
setup. If one of them clearly fails, the wrong operating model usually becomes obvious.
...
Use the matrix below as a path filter, not a promise of profitability. It helps you decide which
model is realistic for your hardware, stake access, and operating capacity under current network
conditions.
...
AI inference lowers the stake barrier, not the operating burden. Demand shifts between pipelines,
cold models waste time and money, and weak VRAM planning can erase the advantage of getting started
without active-set stake.
