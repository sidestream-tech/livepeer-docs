...
The active network range at time of writing is roughly **500 to 2,000 wei per pixel**. Prices below 500 wei per pixel are typically below cost. Prices above 2,000 wei per pixel are above most gateway caps and receive no work.

{/* REVIEW: Verify current active network price range with Rick / tools.livepeer.cloud. The 500-2,000 wei figure is from config-draft research (March 2026) and may have shifted. */}
...
To price in USD rather than wei, append the `USD` suffix. go-livepeer converts the value to wei at runtime using a Chainlink ETH/USD price feed on Arbitrum:
...
{/* REVIEW: Confirm -priceFeedAddr is not required when using USD notation on Arbitrum mainnet (the default feed should be pre-configured in go-livepeer). Rick to verify. */}

### Surveying the market

Check [Livepeer Explorer](https://explorer.livepeer.org/orchestrators) for the current price distribution across the active set. Active orchestrators are listed with their advertised price. Position in the **25th to 75th percentile** of the active set to attract work without racing to the bottom.
...
{/* REVIEW: Confirm -autoAdjustPrice is production-stable and not gated behind any feature flag. Rick to verify current go-livepeer source. */}
...
{/* REVIEW: Confirm typical price ranges for each pipeline with current network data from tools.livepeer.cloud/ai/network-capabilities. The wei figures above are from diffusion-pipeline-setup.mdx (March 2026) and should be treated as illustrative baselines, not current market rates. */}
...
USD-denominated AI pricing is particularly useful for LLM pipelines where per-token economics are more intuitive than wei:
...
{/* REVIEW: Confirm exact flag syntax for -pricePerGateway (format: flag value as JSON map, or address:price pair, or separate flag per gateway). Rick to verify current go-livepeer CLI interface for this flag. The flag name is confirmed in gateway-relationships.mdx source but the input format is not documented in available source files. */}
