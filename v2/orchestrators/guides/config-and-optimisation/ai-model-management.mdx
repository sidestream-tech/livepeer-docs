...
**Cold** means the container exists but the weights are not in VRAM. The first request triggers a model load — typically 10 to 60 seconds depending on model size and NVMe storage speed — before inference begins.

### Impact on job routing

Gateways track first-response latency per orchestrator. Nodes with consistently fast response times win more jobs. For latency-sensitive pipelines — particularly `text-to-image` and `image-to-image` — cold loading creates a competitive disadvantage on the first request of each session.
...
{/* REVIEW: Confirm whether the one-warm-model-per-GPU Beta constraint is still in effect. Rick / ai-runner team to verify — this may have been lifted in a recent release. */}
...
A 24 GB GPU can hold one large diffusion model warm, with a small pipeline (Whisper or BLIP) warm on the same card if using a multi-GPU system. Do not attempt to warm multiple large diffusion models on a single 24 GB GPU — the Beta constraint prevents this, and VRAM would be insufficient.

<CustomDivider middleText="Model Rotation" />

## Model rotation by demand

Demand on the Livepeer AI network shifts over time. A model in high demand one week may see reduced traffic the next as new models are listed or gateway preferences change. Warm model selection should track demand, not personal preference.
...
Restart only the AI worker container — not the full go-livepeer process — to minimise downtime:

```bash icon="terminal"
docker restart <ai-runner-container-name>
```

{/* REVIEW: Confirm that restarting only the AI runner container (not go-livepeer) correctly picks up aiModels.json changes. Rick to verify. */}
  ...
  <Tabs>
    ...
    <Tab title="DEEPCACHE" icon="database">
      **DEEPCACHE** caches intermediate diffusion steps to reduce redundant recomputation across inference calls. Delivers up to 50% faster inference with minor quality reduction (slight reduction in fine detail at high step counts).
      ...
      Quality degradation is more visible at low step counts. Do not use with Lightning or Turbo model variants — these are already step-optimised for 1 to 4 inference steps, and DEEPCACHE degrades their output with no speed benefit.
...
<Warning>
  Do not apply DEEPCACHE to Lightning or Turbo models (e.g. `ByteDance/SDXL-Lightning`, `SG161222/RealVisXL_V4.0_Lightning`). These models operate at 1 to 4 inference steps. DEEPCACHE's caching mechanism is designed for 20+ step models — applying it to low-step models degrades output quality without producing meaningful speed gains.
</Warning>
...
If a warm model does not appear registered after 5 minutes, check for:
...
- Model download still in progress — warm models must be fully downloaded before the container can load them
