...
This page covers the operational mechanics of how LPT rewards flow, how to call `Reward()` reliably, the economics of when calling reward is profitable, and how ETH fees move from job to wallet. It assumes you have read [Earnings Explained](/v2/orchestrators/guides/staking-and-rewards/earnings) and have an active orchestrator node.
...
<Warning>
  If your node is offline at round initialisation, that round's reward is missed. There is no retry. For production orchestrators, use systemd or Docker restart policies to keep the node running continuously.
</Warning>
...
Once your stake is large enough that automatic reward calls are clearly profitable:
...
For the full probabilistic micropayment lifecycle, ticket structure, and Redeemer operation, see [Payments](/v2/orchestrators/guides/staking-and-rewards/payments).
...
**Node:** 10,000 LPT total stake (3,000 self-bonded + 7,000 delegated), active set, `rewardCut` = 15%, `feeShare` = 40%
...
{/* REVIEW: Alison/Mehrdad to verify current inflation rate, total supply, and active stake figures — numbers above are illustrative approximations */}
...
{/*
  PURPOSE:
  Journey step: "How do reward calling and fee distribution work?"
  Guide covering the mechanics of calling rewards, fee distribution,
  and optimising reward calling frequency.

  CONTENT SHOULD INCLUDE:
  - Round-based reward calling: when, how, gas costs
  - Reward calling configuration (-rewardAddr, -reward)
  - Automatic vs manual reward calling
  - Fee distribution: reward cut and fee cut parameters
  - Setting competitive cut rates
  - Gas optimisation for reward calling on Arbitrum
  - Monitoring reward calling success (missed rounds)

  SECTION HOME: Guides > Staking & Rewards

  JOURNEY POSITION:
  1. Earnings - "How do I earn?"
  2. Rewards and Fees (this page) - "How do rewards and fees work?"
  3. Attracting Delegates - "How do I grow my stake?"
  4. Governance - "How do I participate in governance?"

  CROSS-REFS:
  - Staking & Rewards > Earnings - earnings overview
  - Setup > Connect and Activate - initial reward calling setup
  - Monitoring & Tools > Metrics - reward calling metrics
  - Resources > Arbitrum RPC - RPC for reward transactions
*/}
