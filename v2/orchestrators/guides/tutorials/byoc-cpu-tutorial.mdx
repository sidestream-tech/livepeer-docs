...
<Steps>
  <Step title="Acquire ETH on Arbitrum One">
    ...
    - Buy directly on Arbitrum via exchanges listed at the [Arbitrum Exchanges reference](/v2/gateways/references/arbitrum-exchanges)
    - Buy on a CEX (Coinbase, Binance) and withdraw directly to Arbitrum One

    <Warning>
    Confirm current ETH deposit requirements in [#lounge on Discord](https://discord.gg/livepeer) or the [on-chain requirements page](/v2/gateways/run-a-gateway/requirements/on-chain) before depositing — amounts can change as ETH/USD rates shift.
    </Warning>
  </Step>
  ...
  <Step title="Set up an Arbitrum RPC endpoint">
    ...
    - Community Arbitrum RPC (see [Arbitrum RPC reference](/v2/gateways/references/arbitrum-rpc))
...
<CardGroup cols={2}>
  ...
  <Card title="Monitoring and optimisation" href="/v2/gateways/run-a-gateway/monitor/monitor-and-optimise" icon="gauge">
