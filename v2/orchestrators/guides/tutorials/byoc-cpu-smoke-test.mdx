...
<StyledSteps>
  ...
  <StyledStep title="Install go-livepeer (if not already installed)">
    ...
    {/* REVIEW: Confirm latest release version tag and correct archive filename. Rick to verify the download URL pattern is stable across releases. */}
  ...
  <StyledStep title="Start the orchestrator (off-chain mode)">
    ...
    {/* REVIEW: Confirm -byoc, -byocContainerURL, -byocModelID flags are stable in current go-livepeer release. Rick to verify exact flag names. */}
