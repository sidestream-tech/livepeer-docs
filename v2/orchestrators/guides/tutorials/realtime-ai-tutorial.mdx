...
This tutorial sets up a working `live-video-to-video` pipeline using the Cascade architecture and ComfyStream. By the end, a live video stream enters the orchestrator, StreamDiffusion transforms each frame in real time, and the output stream is viewable. Estimated time: **3 hours** (most of this is model download time).
...
<StyledTable variant="bordered">
  ...
  <tbody>
    ...
    <TableRow>
      ...
      <TableCell>The orchestrator must already be running. Start from AI Earning Quickstart if needed.</TableCell>
    </TableRow>
    <TableRow>
...
<Warning>
  GPUs below 24 GB VRAM (RTX 3080 10 GB, RTX 3060 12 GB) are typically insufficient for real-time inference at acceptable frame rates. StreamDiffusion's stream batch buffers, model weights, and ControlNet adapters combined exhaust available VRAM on these cards.
</Warning>
...
The GPU table should appear inside the container output. If it does not, re-install the NVIDIA Container Toolkit before proceeding.
...
{/* REVIEW: Confirm the `live-base` tag is current and stable in the livepeer/ai-runner Docker Hub repository. Rick to verify the correct tag name for production use. */}
...
{/* REVIEW: Confirm the ComfyStream download script name and path are current in the livepeer/comfystream repository. Rick to verify the download script invocation. */}
...
The `model_id` for the live pipeline refers to the ComfyUI workflow or pipeline name, not a HuggingFace model ID in the standard batch sense. The models themselves are loaded inside the ComfyStream container.
...
{/* REVIEW: Confirm `model_id: "streamdiffusion"` is the correct value for the live-video-to-video pipeline in current go-livepeer. Rick / j0sh to verify exact model_id format. */}

`price_per_unit` for the live pipeline is charged per frame (unlike batch pipelines which charge per pixel or per millisecond). Set a value that is at or below what gateways have configured in their `-maxPricePerCapability` for `live-video-to-video`. Check current rates at [tools.livepeer.cloud/ai/network-capabilities](https://tools.livepeer.cloud/ai/network-capabilities).

<CustomDivider middleText="Step 5: Start the Orchestrator" />

## Step 5: Start go-livepeer with live AI flags

If the orchestrator is already running with `-aiWorker` from a previous setup, stop it and restart with the updated `aiModels.json`. If starting fresh:
...
Expected (may take 5 to 10 minutes):
```
Starting AI worker
...
Start a gateway that can route `live-video-to-video` jobs to the orchestrator. For a local test, use off-chain mode:
...
<Note>
  For a production gateway routing live AI jobs on-chain, configure `-maxPricePerCapability` with a cap for `live-video-to-video`. The gateway will not route to orchestrators priced above this cap, regardless of hardware capability.
</Note>
...
If the manifest is returned (non-empty file), the live pipeline is processing frames and delivering output.
...
{/* REVIEW: Confirm gateway output HLS endpoint path for live-video-to-video jobs. j0sh to verify the correct URL pattern for the output manifest in current go-livepeer. */}
...
Monitor frame processing times in the orchestrator logs. At 30 fps, each frame should be processed in under 33 ms. If frame times exceed this consistently, the pipeline is not keeping pace with the incoming stream:
...
- The model may be too slow for the target fps. StreamDiffusion at 2 steps is the minimum viable configuration for 30 fps on an RTX 4090. Try reducing output resolution.
...
If this returns `False`, CUDA is not accessible inside the container. Re-install the NVIDIA Container Toolkit.
...
Payment for live streams uses an interval-based model rather than per-frame: the gateway sends periodic PM tickets at a configurable interval (`-livePaymentInterval`, default 5 seconds) rather than one ticket per frame. This reduces payment overhead for continuous streams.
