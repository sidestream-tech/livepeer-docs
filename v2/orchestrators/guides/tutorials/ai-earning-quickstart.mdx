...
<Tip>
  AI inference routing is capability-based, not stake-based. A node with a 24 GB GPU and a warm model can receive AI jobs immediately after registration - no active set membership, no minimum LPT stake required.
</Tip>
...
<StyledSteps>
  ...
  <StyledStep title="Verify the AI runner can access the GPU">
    ...
    Expected: the `nvidia-smi` table showing your GPU, driver version, and VRAM. If this fails, re-install the NVIDIA Container Toolkit.
...
{/* REVIEW: Confirm dl_checkpoints.sh script name and invocation pattern is current in the latest ai-runner image. Rick / ai-runner team to verify. */}
...
<StyledSteps>
  ...
  <StyledStep title="Watch for AI worker startup">
    ...
    If `Warm model loaded` does not appear within 10 minutes, check for errors:
...
<StyledSteps>
  ...
  <StyledStep title="Activate on-chain (if not already an orchestrator)">

    If this is a new node (not an existing video orchestrator), run `livepeer_cli` to register:
    ...
    {/* REVIEW: Confirm the exact livepeer_cli activation option name in current release. Rick to verify. */}
  ...
  <StyledStep title="Verify external network registration">
    ...
    If the node does not appear after 10 minutes, inspect container status:
...
{/* REVIEW: Confirm exact local test endpoint path and request body format for the orchestrator's direct AI HTTP API. Rick / j0sh to verify current endpoint. */}
...
<Note>
  The first inference after startup may be slower than subsequent ones as the CUDA kernels warm up. This is normal. From the second request onward, inference should complete in under 5 seconds for SDXL-Lightning at 512×512.
</Note>
...
**No active set membership was required.** AI inference routing checks capability advertisement and price, not stake rank. A new node with a warm model and a competitive price can receive AI jobs on the same day it registers.
