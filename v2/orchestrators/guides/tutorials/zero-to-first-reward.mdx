...
<Tip>
  This tutorial covers one complete path: video transcoding to first LPT reward. Nothing else is included. AI pipelines, dual mode, and pool joining are separate paths documented elsewhere. Complete this tutorial first, then add to it.
</Tip>

---

At the end of this tutorial the orchestrator node is running on Arbitrum One mainnet, processing video transcoding jobs, and has successfully called `Reward()` to claim its first LPT inflation allocation. Estimated time: **4 to 8 hours** — the range is wide because acquiring LPT on Arbitrum may take hours depending on exchange availability and bridge confirmation times.
...
<StyledTable variant="bordered">
  ...
  <tbody>
    <TableRow>
      ...
      <TableCell>Production GPU transcoding is Linux-first. macOS can run the binary for CLI access only.</TableCell>
    </TableRow>
    <TableRow>
    ...
    <TableRow>
      ...
      <TableCell>Free tier at Alchemy (`arb-mainnet.g.alchemy.com`) or Infura. Must be Arbitrum One, not Ethereum L1.</TableCell>
    </TableRow>
    <TableRow>
    ...
    <TableRow>
      ...
      <TableCell>Enough to stake. The active set is the top 100 orchestrators by total stake. Check the current threshold at [Livepeer Explorer](https://explorer.livepeer.org/orchestrators). Acquiring LPT may take hours - plan for this before starting.</TableCell>
    </TableRow>
  </tbody>
</StyledTable>

<Warning>
  Acquiring LPT on Arbitrum One can take hours depending on exchange availability and bridge confirmation times. Do not start this tutorial unless LPT is already in your Arbitrum wallet, or budget the waiting time explicitly.
</Warning>
...
<Note>
  This is the waiting step. Bridge ETH from Ethereum L1 at [bridge.arbitrum.io](https://bridge.arbitrum.io) (confirmation: 10-15 minutes) or buy directly from a centralised exchange with Arbitrum One withdrawal support. LPT acquisition depends on your exchange options and may take hours.
</Note>
...
{/* REVIEW: Confirm the exact livepeer_cli menu option for viewing account balances in the current release. Rick to verify option name/number. */}
...
<StyledSteps>
  ...
  <StyledStep title="Activate as an orchestrator">
    ...
    {/* REVIEW: Confirm exact livepeer_cli menu item name for activation in current release. Rick to verify "Invoke multi-step become an orchestrator" is current wording. */}
...
- **Status** shows as Active (may take up to one full round to appear — approximately 22 hours)
...
<Note>
  Entry into the active set is not immediate after activation. The node must be in the top 100 by total stake at the start of the next round. If total stake is below the 100th orchestrator's stake, the node is registered but not active. Monitor your position on Explorer and acquire more delegated LPT if needed.
</Note>
...
<Note>
  A node that was not in the active set at the start of a round receives no reward for that round. If the first reward shows zero, confirm active set membership on Explorer and wait for the next round.
</Note>
...
ETH fee income is separate and arrives as gateways route transcoding jobs to the node. Fee income scales with job volume, not just stake.
