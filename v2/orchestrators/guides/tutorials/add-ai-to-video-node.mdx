...
**Prerequisites:** A working video orchestrator already running on Arbitrum One mainnet. If the node is not yet set up, start with <LinkArrow href="/v2/orchestrators/guides/tutorials/zero-to-first-reward" label="Zero to First Reward" newline={false} />.

<CustomDivider middleText="What Changes" />

## What changes and what does not

Video transcoding uses NVENC and NVDEC - the fixed-function hardware blocks built into NVIDIA GPUs. AI inference uses CUDA compute cores. These are separate hardware resources on the same die. Adding AI workloads does not displace video transcoding capacity or interfere with NVENC sessions.
...
<StyledTable variant="bordered">
  ...
  <tbody>
    ...
    <TableRow>
      ...
      <TableCell>**NEW if not present** - required for AI runner containers</TableCell>
    </TableRow>
  </tbody>
</StyledTable>
...
{/* REVIEW: Validate VRAM figures against model-demand-reference.mdx when that page merges. Rick to confirm per-pipeline VRAM floors. */}
...
{/* REVIEW: Confirm dl_checkpoints.sh script invocation pattern is current. Rick / ai-runner team to verify. */}
...
Set `price_per_unit` based on current market rates at [tools.livepeer.cloud/ai/network-capabilities](https://tools.livepeer.cloud/ai/network-capabilities). Gateways will not route AI jobs to the node if the price exceeds their configured cap.
...
The `-v /var/run/docker.sock:/var/run/docker.sock` mount is required for Docker-out-of-Docker - go-livepeer uses the Docker daemon to start AI runner containers. Add it if it was not already present in the video-only startup command.

<Warning>
  `-aiModelsDir` must be the host path (`~/.lpData/models`), not a container path. Docker mounts this directory into each AI runner container it creates.
</Warning>
...
<StyledSteps>
  ...
  <StyledStep title="Test AI inference">
    ...
    {/* REVIEW: Confirm local inference endpoint path and request format for current go-livepeer release. Rick / j0sh to verify. */}
...
Video transcoding routes through NVENC/NVDEC (dedicated silicon - no CUDA usage, negligible VRAM). AI inference routes through CUDA compute cores with model weights loaded in VRAM. The two workloads do not contend for the same hardware resources.
...
Both streams use the same wallet, the same `Reward()` call for LPT inflation, and the same on-chain registration. No separate accounts or registrations are needed for the AI income stream.
