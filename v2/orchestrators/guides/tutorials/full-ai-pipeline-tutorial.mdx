...
<Tip>
  The gateway handles routing and payment negotiation. The orchestrator handles compute. This tutorial runs both on one machine, off-chain, so you can watch a real inference request travel through both sides and return a result - without a wallet or on-chain registration.
</Tip>
...
{/* REVIEW: Confirm dl_checkpoints.sh invocation syntax against current ai-runner image. Rick / ai-runner team to verify. */}
...
<Note>
  The remote signer at `signer.eliteencoder.net` is a community-hosted service for testing. Confirm availability in [#local-gateways on Discord](https://discord.gg/livepeer) if connection errors occur.
</Note>
...
Send a `text-to-image` request through the gateway (port 8936, not directly to the orchestrator on 8935):
...
{/* REVIEW: Confirm the gateway's AI inference endpoint path is /text-to-image for off-chain gateway in current go-livepeer release. j0sh / Rick to verify. */}
...
5. **The orchestrator** returned the result to the gateway and evaluated the payment ticket (in off-chain mode, settlement is handled by the remote signer rather than the Arbitrum TicketBroker).
...
<CardGroup cols={2}>
  ...
  <Card title="Realtime AI Tutorial" icon="video" href="/v2/orchestrators/guides/tutorials/realtime-ai-tutorial" arrow horizontal>
    The live video-to-video pipeline - streaming AI rather than batch inference.
  </Card>
</CardGroup>
