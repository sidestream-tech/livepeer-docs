...
{/*
  PURPOSE:
  "Something broke - how do I fix it?" Symptom-based guide indexed by what the
  operator observes. Not receiving video jobs (7 checks). Not receiving AI jobs
  (5 checks). OrchestratorCapped (session limit). GPU OOM. Missed reward calls.
  NVENC session limit. Networking unreachable. Price not updated on-chain. AI model
  download failures. Diagnostic command reference. Escalation: Discord, Forum, GitHub.

  PLAN TARGET: troubleshooting (keep)
  UPDATE NEEDED: Add symptom quick-nav index at top.

  SECTION: Monitoring & Tools → "How do I keep it running?"
  JOB STORIES: J7 (incident response)

  CROSS-REFS:
  - Resources > FAQ - quick answers to common questions
  - Monitoring & Tools > Metrics and Alerting - diagnosing via metrics
  - Setup > Verify - verification steps that may help diagnose
*/}
