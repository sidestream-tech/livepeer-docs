...
{/*
  PURPOSE:
  "How do I set up monitoring that tells me when something is wrong?" Prometheus:
  -monitor flag, endpoint at localhost:7935/metrics, -metricsPerStream. Key metrics:
  transcoding_success_rate, session_count, session_max_limit, ai_job_success_rate,
  go_memstats_heap_bytes. Grafana dashboard config. Alert rules: missed reward calls,
  session saturation >80%, high error rate, OOM approaching, zero sessions >1 hour.
  Docker monitoring for AI runner containers.

  PLAN TARGET: metrics-and-alerting (consider rename)
  SECTION: Monitoring & Tools → "How do I keep it running?"
  JOB STORIES: J7 (proactive monitoring)

  CROSS-REFS:
  - Config & Optimisation > Capacity Planning - metrics that indicate capacity issues
  - Setup > Initial Monitoring - basic setup-time monitoring
