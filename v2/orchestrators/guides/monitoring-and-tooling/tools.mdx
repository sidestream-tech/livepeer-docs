...
{/*
  PURPOSE:
  "What tools exist for managing my orchestrator?" Livepeer Explorer (live stake,
  rank, sessions, fees). tools.livepeer.cloud (AI capabilities, pipeline demand,
  pricing benchmarks). livepeer_cli (local management). Community tools: Speedy
  Bird monitoring stack, Titan Node pool client, Orchestrator Watcher bots.
  Diagnostic endpoints: /status, /getOrchestrator, /getNetworkCapabilities, /metrics.

  PLAN TARGET: operator-toolbox (consider rename)
  SECTION: Monitoring & Tools → "How do I keep it running?"
  JOB STORIES: J7 (tool discovery)

  CROSS-REFS:
  - Monitoring & Tools > Explorer Operations - detailed Explorer usage
  - Monitoring & Tools > Metrics and Alerting - Prometheus setup
