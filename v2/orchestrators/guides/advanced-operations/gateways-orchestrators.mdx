...
Your orchestrator does not pull work from the network. Gateways push work to you — but only if they can find you, only if you support what they need, and only if your price fits within their budget. Understanding this pipeline is what separates operators who consistently receive work from those who are technically running but idle.
...
See [Configure Pricing](/v2/orchestrators/setup/r-configure) for how to adjust your prices.
</Accordion>
...
## Gateway Loki API — understanding why you are (or are not) being selected
...
{/* REVIEW: Verify loki.livepeer.report is still live and returning data. The endpoint returned no response during March 2026 sandbox verification — may be a network restriction in the test environment. Rick to confirm current availability before publishing. */}

<CustomDivider />

## Debugging: not receiving work

Use this checklist in order when you are in the active set but not receiving jobs:
...
<CardGroup cols={2}>
  <Card title="Configure Pricing" icon="tag" href="/v2/orchestrators/setup/r-configure">
  ...
  <Card title="Orchestrator Tools" icon="toolbox" href="/v2/orchestrators/guides/monitoring-and-troubleshooting/tools">
...
{/*
  PURPOSE:
  Journey step: "How do gateways discover and interact with my orchestrator?"
  Guide explaining the gateway-orchestrator relationship from the orchestrator
  perspective: how to be discovered, selected, and serve work effectively.

  CONTENT SHOULD INCLUDE:
  - How gateways discover orchestrators (on-chain registry, -orchAddr)
  - Selection criteria: price, latency, reliability, capabilities
  - Optimising for gateway selection (competitive pricing, uptime)
  - AI service registry and capability advertisement
  - Working with specific gateways (direct relationships)
  - Commercial orchestrator relationships (Livepeer Studio, etc.)
  - Price negotiation and the PM ticket flow

  SECTION HOME: Guides > Advanced Operations

  JOURNEY POSITION:
  1. O-T Setup - "How do I split O and T?"
  2. Run a Pool - "How do I run a pool?"
  3. Working with Gateways (this page) - "How do gateways interact with me?"
  4. Large-Scale Operations - "How do I run at scale?"

  CROSS-REFS:
  - Gateway Tab > Advanced Operations > Orchestrator Selection - gateway side
  - Payments & Pricing - pricing that affects selection
  - Concepts > Capabilities - what you advertise
  - Gateway Tab > Node Pipelines > AI Pipelines - AI routing from gateway side
*/}
