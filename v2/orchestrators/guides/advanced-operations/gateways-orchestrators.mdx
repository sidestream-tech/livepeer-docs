...
| Factor | How it works | What you control |
|---|---|---|
| **Capability matching** | Does your node support the requested pipeline, resolution, or AI model? | `aiModels.json`, transcoding profiles |
| **Price constraints** | Your price must be at or below the gateway's configured maximum | `-pricePerUnit` for transcoding; per-capability pricing for AI |
| **Performance score** | Historical success rate, latency, and failure rate tracked per orchestrator | Uptime, hardware quality, network speed |
| **Stake weight** | For video transcoding, higher stake increases selection probability (configured via gateway `-selectStakeWeight`) | Total stake (self + delegated) |
| **Random factor** | Gateways introduce randomness (`-selectRandFreq`) to prevent centralisation | Not directly controllable |
...
<Accordion title="1. Price competitively">
...
<Accordion title="2. Keep your service URI correct and reachable">
...
<Accordion title="3. Register your AI capabilities correctly">
...
<Accordion title="4. Maintain high performance scores">
...
<Accordion title="5. Build stake for video transcoding">
...
<CardGroup cols={2}>
  ...
  <Card title="Orchestrator Tools" icon="toolbox" href="/v2/orchestrators/guides/monitoring/tools">
