...
|---|---|---|
| Per-session tracking | Log sessions handled per worker ETH address; pay proportionally at period end | Small pools with a few trusted workers |
| Custom pool client | Build tooling that tracks address + nickname + sessions; automate periodic payouts | Public-facing pools (Titan Node's model) |
| Manual distribution | Calculate shares manually; send ETH periodically | Testing and very small pools only |
...
<Accordion title="Worker connection management">
...
<Accordion title="NVENC session caps on consumer GPUs">
...
<Accordion title="Session routing and load balancing">
...
<Accordion title="Node updates and downtime">
...
<Accordion title="Payout and worker communication">
...
<CardGroup cols={2}>
  ...
  <Card title="Earnings and Economics" icon="chart-line" href="/v2/orchestrators/concepts/earnings">
