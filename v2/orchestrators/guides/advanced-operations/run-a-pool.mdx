...
If you are looking to **join** an existing pool as a worker, see [Join a Pool](/v2/orchestrators/guides/setup-paths/join-a-pool) instead.
...
<CardGroup cols={2}>
  ...
  <Card title="Earnings and Economics" icon="chart-line" href="/v2/orchestrators/guides/staking-and-rewards/earnings">
...
{/*
  PURPOSE:
  Journey step: "How do I run an orchestrator pool?"
  Guide for operators who want to run a pool: accepting external transcoders,
  managing workers, distributing fees, and pool economics.

  CONTENT SHOULD INCLUDE:
  - Pool operator responsibilities
  - Configuration for accepting pool workers
  - Fee distribution to pool workers
  - Pool economics: operator take vs worker share
  - Managing worker reliability and performance
  - Communication with pool members
  - Comparison with solo operation

  SECTION HOME: Guides > Advanced Operations

  JOURNEY POSITION:
  1. O-T Setup - "How do I split O and T?"
  2. Run a Pool (this page) - "How do I run a pool?"
  ...
  - Resources > Community Pools - existing pools
  - Staking & Rewards > Earnings - pool earnings distribution
  - Advanced Operations > O-T Setup - pool with split architecture
*/}
