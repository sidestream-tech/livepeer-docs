...
- go-livepeer is installed and running as a transcoding orchestrator on Arbitrum mainnet (see [Install go-livepeer](/v2/orchestrators/setup/install-go-livepeer) and [Get Started](/v2/orchestrators/get-started/quickstart)) {/* REVIEW: confirm target paths */}
...
- Model weights pre-downloaded for the pipeline(s) you want to serve (see [Download AI Models](/v2/orchestrators/advanced/hosting-models)) {/* REVIEW: confirm path */}
...
Check the [AI Pipelines](/v2/orchestrators/advanced/ai-pipelines) documentation for any other pipeline-specific images.
...
Your AI pipelines will appear in the [Livepeer Explorer](https://explorer.livepeer.org) on your orchestrator's profile once on-chain capability advertisement is configured. See [Publish Offerings](/v2/orchestrators/setup/publish-offerings) for that step. {/* REVIEW: confirm path */}
