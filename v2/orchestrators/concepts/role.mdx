...
<CenteredContainer style={{ width: 'fit-content', minWidth: 'fit-content' }}>
  <AccordionGroup>
    <Accordion title="From a Cloud Background?" icon="cloud">
      Running an Orchestrator is similar to operating a managed GPU instance in a cloud environment - except the
      "cloud" is decentralised and the workloads are assigned by a permissionless marketplace.

      Instead of AWS allocating compute capacity, Livepeer Gateways select your node based on price, capability,
      ...
      flowchart LR
          ...
          style Cloud fill:#0d0d0d,stroke:#2d9a67,stroke-width:1px
          style Livepeer fill:#0d0d0d,stroke:#2d9a67,stroke-width:1px
          style WorkSrc fill:#0d0d0d,stroke:#2d9a67,stroke-width:1px
      ```
    ...
    <Accordion title="From an Ethereum Background?" icon="coin">
      ...
      Ethereum node - but the work is compute rather than consensus.
      ...
      inflation rewards for calling rewards each round. Slash risk exists for poor performance, not double-signing.
      ...
      flowchart LR
          ...
          classDef default fill:#1a1a1a,color:#fff,stroke:#2d9a67,stroke-width:2px
          style ETH fill:#0d0d0d,stroke:#2d9a67,stroke-width:1px
          style LP fill:#0d0d0d,stroke:#2d9a67,stroke-width:1px
      ```
    ...
    <Accordion title="Neither? Here's the clearest mental model." icon="microchip">
      ...
      flowchart LR
          ...
          style Net fill:#0d0d0d,stroke:#2d9a67,stroke-width:1px
          style Earn fill:#0d0d0d,stroke:#2d9a67,stroke-width:1px
      ```
...
<CenteredContainer style={{ width: '80%' }}>
...
Orchestrators can be deployed in several configurations. The right setup depends on hardware scale,
workload type, and operational preference.
...
<AccordionGroup>
  <Accordion title="The Miner - Can I earn from my GPU?" icon="microchip">
    If you already have GPU hardware running - mining, rendering, or sitting idle - Livepeer lets you
    direct that capacity at video transcoding or AI inference and earn ETH for the work.
  ...
  <Accordion title="The Easy Earner - Simplest path?" icon="bolt">
    If you want to participate without managing LPT staking or on-chain activation, joining an existing
    pool is the fastest path. Bring GPU hardware; the pool handles the rest.
  ...
  <Accordion title="The Pro Operator - Adding AI to an existing setup?" icon="server">
    If you already run video transcoding and want to add AI inference workloads, the Orchestrator model
    supports dual-workload operation from a single node. New capabilities are advertised automatically
    once configured.
...
<CardGroup cols={2}>
  <Card title="Orchestrator Capabilities" icon="gears" href="/v2/orchestrators/concepts/capabilities" arrow horizontal>
    Workload types, job execution, and what Orchestrators can and cannot do.
  </Card>
