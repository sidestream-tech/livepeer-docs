...
| Cost | Description |
|---|---|
| **GPU hardware** | Purchase or lease NVIDIA GPUs. Consumer cards (RTX 3090/4090) for small operators; data-centre cards (A100, H100) for large operations |
| **Hosting** | Colocation, cloud, or home setup. Includes power, cooling, and network |
| **Bandwidth** | Video transcoding is bandwidth-intensive; AI inference less so |
| **Energy** | GPU power draw under load (200-400W per card is typical) |
| **ETH for gas** | On-chain transactions (registration, reward calls, ticket redemption) require small amounts of ETH on Arbitrum |
| **LPT for staking** | Must acquire and stake LPT to register and compete for work |

### Operational costs

| Cost | Description |
|---|---|
| **Uptime commitment** | Node must be online to receive jobs and call `reward()` each round |
| **Maintenance** | Software updates, GPU driver updates, monitoring |
| **Delegation management** | Competitive reward cut and fee cut settings to attract delegators |
| **Risk** | Hardware failure, downtime, missed reward calls, ETH price volatility |
