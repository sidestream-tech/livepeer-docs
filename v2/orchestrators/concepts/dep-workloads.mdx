...
status: current
---
{/* Covers the compute domains orchestrators support.

Video
Video transcoding and related media processing workloads.

AI
AI inference, pipelines, and custom compute workloads, including containerized and BYOC-style jobs. */}
...
<Note>
The `upscale` and `text-to-speech` VRAM requirements are not confirmed in this draft — they will be filled before publication. Check [AI Pipelines](/v2/orchestrators/advanced/ai-pipelines) for current per-pipeline specs.
</Note>
...
Configured via the `aiModels.json` file. Each entry specifies the pipeline name, model ID, pricing, whether the model is kept warm, and the URL of the runner container. See [Configure AI Pipelines](/v2/orchestrators/advanced/ai-pipelines) for the configuration reference.
...
Configured via `aiModels.json` and requires a running ComfyStream instance. See [Set Up Real-time AI Pipelines](/v2/orchestrators/advanced/ai-pipelines#live-video-to-video-cascade) for the setup guide.
...
Requires deploying the Ollama runner container alongside your orchestrator. Configured via `aiModels.json` with `"pipeline": "llm"`. See [Set Up the LLM Pipeline](/v2/orchestrators/advanced/ai-pipelines#llm-inference-ollama-runner) for the deployment guide.
...
{/* REVIEW: Current file had CardGroup quickstart links — not present in new draft. Human to decide whether to include. */}
