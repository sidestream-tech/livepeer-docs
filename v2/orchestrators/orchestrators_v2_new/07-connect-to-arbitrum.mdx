...
<Note>
  Livepeer runs on **Arbitrum One**, an Ethereum Layer 2 network. Arbitrum
  handles the protocol's staking, payments, and governance contracts. Your
  orchestrator node needs a live connection to Arbitrum to activate, receive
  payments, and submit on-chain transactions. This connection is provided by an
  RPC endpoint — either from a hosted provider or a self-hosted node.
</Note>
...
- Your Ethereum account has a small amount of ETH on Arbitrum One for gas. Typical activation transactions cost fractions of a cent; a few dollars of arbETH is sufficient to get started.
  {/* <!-- REVIEW: check current gas costs with Mehrdad/community --> */}
...
<Note>
  If you omit `-ethAcctAddr`, go-livepeer will create a new Ethereum account on
  first run and prompt you for a passphrase to protect the keystore. The private
  key is stored at `~/.lpData/arbitrum-one-mainnet/keystore`.
</Note>
...
- [Activate on the Network](/v2/orchestrators/setup/publish-offerings) — next step: register on-chain and join the active set
  {/* REVIEW: confirm path */} -[Frequently Asked Questions](/v2/resources/faq) — Arbitrum RPC error reference
