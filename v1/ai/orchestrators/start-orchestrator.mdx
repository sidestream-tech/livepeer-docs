...
description: "Launch your combined AI Orchestrator node using Docker or pre-built binaries on the Livepeer AI network."
---
    ...
    <Tab title="Use Docker (Recommended)">
        <Steps>
            ...
            <Step title="Launch an (off-chain) AI Orchestrator">
                ...
                Lastly, the `-nvidia` can be configured in a few ways. Use a comma separated list of GPUs i.e. `0,1` to activate specific GPU slots, each GPU will need its own config item in `aiModels.json`. Alternatively we can use `"all"` to activate all GPUs on the machine with a single model loaded in `aiModels.json` (Warning: If different RAM size GPUs are installed it may cause containers to fail if they have less than the required RAM).
